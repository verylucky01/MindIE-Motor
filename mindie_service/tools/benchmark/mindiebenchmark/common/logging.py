...
from mindiebenchmark.common.utils import print_value_warn

MODULE_NAME = "mindie-benchmark"
UNSET_LOGGER = "NULL"
...
DEFAULT_LOG_PARAMS = LogParams(
    ...
    to_console=False,
    verbose=True,
    rotate_options={FILE_SIZE: 20 * 1024 * 1024, FILE_COUNT: 64, FILE_PER_PROCESS: 10},
)
...
def _create_log_file(log_file):
    ...
    if not os.path.exists(log_file):
        ...
        if len(clean_path) > 1024:
            err_msg = (
                f"Path of log file is too long, it should not exceed 1024 character."
            )
            raise ValueError(err_msg)
...
def _close_logger(parent_directory: str, base_filename, ts):
    ...
    if os.path.exists(base_filename):
        _change_to_readonly(base_filename)
...
class CustomRotatingFileHandler(RotatingFileHandler):
    ...
    def rotate(self, source, dest):
        ...
        new_filename = os.path.join(parent_directory, f"{MODULE_NAME}_{pid}_{self.ts}.{self.log_id:02d}log")
    ...
    def close(self):
        if self.stream:
            ...
            _close_logger(
                parent_directory=parent_directory, base_filename=self.baseFilename, ts=self.ts
            )
            self.stream = None
            # limit total file nums
            if self.backupCount > 0:
...
class Logger(metaclass=Singleton):
    MODULE_KEY_NAME = "benchmark"
    MIN_FILE_SIZE = 1 * 1024 * 1024
    MAX_FILE_SIZE = 500 * 1024 * 1024
    MIN_FILE_PER_PROCESS = 1
    MAX_FILE_PER_PROCESS = 64

    def __init__(self, log_params: LogParams = DEFAULT_LOG_PARAMS):
    ...
    @classmethod
    def get_benchmark_log_string_from_env(
        cls, env_values: dict, parse_type: str
    ) -> str:
        """
        解析字符串类环境变量
        1、解析日志路径环境变量
        export MINDIE_LOG_PATH="llm: /home/working; benchmark: /path"
        默认 ~/mindie/log
        2、解析日志等级环境变量
        export MINDIE_LOG_LEVEL="llm: error; benchmark: debug"
        默认 info
        """

        def check_path(path_value, default_value):
            pattern = r"^(~|/[^/ ]*|[^/ ~][^ ]*)+/?$"
            if not isinstance(path_value, str) or not re.match(pattern, path_value):
        ...
        check_fun = {"LOG_PATH": check_path, "LOG_LEVEL": check_level}
        default_values = {"LOG_PATH": "~/mindie/log", "LOG_LEVEL": "INFO"}
        env_value = env_values.get("env_value")
        default_value = check_fun.get(parse_type)(
            env_values.get("default_value"), default_values.get(parse_type)
        )
        if not env_value:
            return default_value
        if ":" not in env_value:
        ...
        if cls.MODULE_KEY_NAME in log_string_dict:
            return check_fun.get(parse_type)(
                log_string_dict[cls.MODULE_KEY_NAME], default_value
            )
        return default_value

    @classmethod
    def get_benchmark_log_bool_from_env(cls, env_values: dict, parse_type: str) -> bool:
        """
        解析日志bool类型环境变量
        export MINDIE_LOG_VERBOSE="llm: true; benchmark: false"
        默认 false
        """
        true_value = "true"
        env_value = env_values.get("env_value")
        ...
        if cls.MODULE_KEY_NAME in log_bool_dict:
            if log_bool_dict.get(cls.MODULE_KEY_NAME) not in BOOL_ENV_CHOICES:
                print_value_warn(
                    MINDIE_PREFIX + parse_type, env_value, default_value, BOOL_ENV_CHOICES
                )
                return is_true_value(default_value)
            return (
                log_bool_dict[cls.MODULE_KEY_NAME].lower() == true_value
                or log_bool_dict[cls.MODULE_KEY_NAME] == "1"
            )
        return is_true_value(default_value)

    @classmethod
    def get_benchmark_log_rotate_from_env(
        cls, env_values: dict, parse_type: str
    ) -> dict:
        """
        解析日志verbose环境变量
        export MINDIE_LOG_ROTATE="llm: -fs 20; benchmark: -fc 4"
        ...
        env_value = env_values.get("env_value")
        default_value = parse_single_rotate_env(
            env_values.get("default_value"), option_dict, parse_type
        )
        if not env_value:
            return default_value
        ...
        if (
            option_dict.get(FILE_SIZE) < cls.MIN_FILE_SIZE
            or option_dict.get(FILE_SIZE) > cls.MAX_FILE_SIZE
        ):
            raise ValueError(
        ...
        if (
            option_dict.get(FILE_PER_PROCESS) < cls.MIN_FILE_PER_PROCESS
            or option_dict.get(FILE_PER_PROCESS) > cls.MAX_FILE_PER_PROCESS
        ):
            raise ValueError(
                "Invalid MINDIE_LOG_ROTATE, file count per process should be in range [1, 64]. "
                "Please check environment value or benchmark config.json"
            )
