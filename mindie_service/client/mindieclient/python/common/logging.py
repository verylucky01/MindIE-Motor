...
from mindieclient.python.common.config import Config
from mindieclient.python.common.file_utils import PathCheck

MODULE_NAME = 'mindie-client'
...
logger_screen = logging.getLogger("client_screen")
...
def print_value_warn(value_type, env_value, default_value, support_choices=None):
    msg = f"[WARNING] Invalid value of client in {value_type}: {env_value}. "
    if support_choices:
...
def init_logger() -> LogParams:
    ...
    log_params.path = Log.get_client_log_string_from_env(log_path_env, "LOG_PATH")
    # set to file
    log_to_file_env = load_log_env(env_type="LOG_TO_FILE")
    log_params.to_file = Log.get_client_log_bool_from_env(log_to_file_env, "LOG_TO_FILE")
    # set to stdout
    log_to_stdout_env = load_log_env(env_type="LOG_TO_STDOUT")
    log_params.to_console = Log.get_client_log_bool_from_env(log_to_stdout_env, "LOG_TO_STDOUT")
    # set log level
    log_level_env = load_log_env(env_type="LOG_LEVEL")
    log_params.level = Log.get_client_log_string_from_env(log_level_env, "LOG_LEVEL")
    # set log verbose option
    log_verbose_env = load_log_env(env_type="LOG_VERBOSE")
    log_params.verbose = Log.get_client_log_bool_from_env(log_verbose_env, "LOG_VERBOSE")
    # set log rotate options
    log_rotate_env = load_log_env(env_type="LOG_ROTATE")
    log_params.rotate_options = Log.get_client_log_rotate_from_env(log_rotate_env, "LOG_ROTATE")
    return log_params
...
def _filter_files(directory, prefix, max_num):
    if not directory:
        directory = os.getcwd()
    is_path_valid, infos = PathCheck.check_path_valid(directory)
    if not is_path_valid:
        raise ValueError(f"{directory} path is invalid")
    all_files = [f for f in os.listdir(directory) if f.startswith(prefix)]
    for f in all_files:
        is_path_valid, infos = PathCheck.check_path_valid(f)
        if not is_path_valid:
            raise ValueError(f"{f} path is invalid")
    ...
    for file in files_to_delete:
        ...
        is_path_valid, infos = PathCheck.check_path_valid(file_path)
        if not is_path_valid:
            raise ValueError(f"{file_path} path is invalid")
        os.remove(file_path)
...
class CustomRotatingFileHandler(RotatingFileHandler):
    ...
    def rotate(self, source, dest):
        ...
        is_path_valid, infos = PathCheck.check_path_valid(new_filename)
        if not is_path_valid:
            raise ValueError(f"{new_filename} path is invalid")
...
class Log(metaclass=Singleton):
    ...
    @classmethod
    def get_client_log_string_from_env(cls, env_values: dict, parse_type: str) -> str:
        """
        解析字符串类环境变量
        1、解析日志路径环境变量
        ...
        def check_path(path_value, default_value):
            if not isinstance(path_value, str) or not PathCheck.check_path_full(os.path.expanduser(path_value))[0]:
    ...
    @classmethod
    def get_client_log_bool_from_env(cls, env_values: dict, parse_type: str) -> bool:
        """
        解析日志bool类型环境变量
        export MINDIE_LOG_VERBOSE="llm: true; client: false"
        默认 false
        """
        true_value = 'true'
        env_value = env_values.get("env_value")
    ...
    @classmethod
    def get_client_log_rotate_from_env(cls, env_values: dict, parse_type: str) -> dict:
        """
        解析日志verbose环境变量
        ...
        if (option_dict.get(FILE_SIZE) < cls.MIN_FILE_SIZE or
            ...
            raise ValueError('Invalid MINDIE_LOG_ROTATE, file size should be in range [1, 500]MB. '
                             'Please check environment value or client config.json')
