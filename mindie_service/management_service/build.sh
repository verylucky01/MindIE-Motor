...
function build_whl ()
{
  echo "start to build om adapter wheel"
  cd "${CURRENT_PATH}"/adapter
  python3 ./setup.py bdist_wheel
  install_dir="$BUILD_MINDIE_SERVICE_INSTALL_DIR"/dist
  mkdir -p "$install_dir"
  cp -v dist/om_adapter-*.whl "${install_dir}"
  rm -rf *.egg-info dist
  cd -

  echo "start to build node_manager wheel"
  cd "${CURRENT_PATH}"/node_health_management/
  python3 ./setup.py bdist_wheel
  install_dir="$BUILD_MINDIE_SERVICE_INSTALL_DIR"/dist
  mkdir -p "$install_dir"
  cp -v dist/node_manager-*.whl "${install_dir}"
  rm -rf *.egg-info dist
  cd -
}
