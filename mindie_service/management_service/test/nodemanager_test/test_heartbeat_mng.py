    ...
    def test_get_ep_status_with_thread(self, mocked_request):
        ...
        def hbm_watch():
            while not running.is_set():
                ...
                if self.last_hbm_state_ != hbm_state_:
                    self.last_hbm_state_ = hbm_state_
                    self.assertEqual(hbm_state_[0], STATUS_MAP[ServiceStatus(self.state_change_map[cur_index]).value][0])
                    self.assertEqual(hbm_state_[1], STATUS_MAP[ServiceStatus(self.state_change_map[cur_index]).value][1])
                sleep(1)
        ...
        while True:
            if cur_index < len(self.state_change_map):
                MOCK_RUNNING_DATA[STATUS_STR], MOCK_RUNNING_DATA[STATUS_STR] = self.state_change_map[cur_index],self.state_change_map[cur_index]
                self._set_dynamic_return(mocked_request, self.engine_map, result_map)
                sleep(3)
            else:
                break
