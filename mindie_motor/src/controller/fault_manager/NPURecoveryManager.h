...
struct ProcessedSwitchFaultInfo {
    ...
    // 生成唯一标识符
    std::string GetUniqueId() const { return faultCode + "|" + switchChipId + "|" + switchPortId + "|" + faultTime; }

    bool operator==(const ProcessedSwitchFaultInfo& other) const {
        return faultCode == other.faultCode && switchChipId == other.switchChipId &&
               switchPortId == other.switchPortId && faultTime == other.faultTime;
    }
};
class NPURecoveryManager {
   public:
    ...
    int32_t Init(std::shared_ptr<NodeStatus> nodeStatus, std::shared_ptr<NodeScheduler> nodeScheduler = nullptr);
   ...
   private:
    ...
    struct InstanceRecoveryInfo {
        ...
        InstanceRecoveryInfo(const std::vector<FaultNodeInfo>& nodes, const std::unordered_set<std::string>& ips)
            : faultNodes(nodes), podIPs(ips), startTime(std::chrono::steady_clock::now()) {}

        // 更新时间戳
        void UpdateTimestamp() { startTime = std::chrono::steady_clock::now(); }
    };
    ...
    // 故障码白名单相关
    std::set<std::string> mFaultCodeWhitelist = {"[0x08520003,na,L2,na]"};
    ...
    std::set<std::string> mDeviceFaultCQECodesWhitelist = {"4C1F8608", "8C1F8608"};
    ...
    void CheckInstancesRecoveryStatus(const InstanceRecoveryData& data, std::vector<uint64_t>& recoveredInstances,
    ...
    void ProcessRecoveredAndTimeoutInstances(const std::vector<uint64_t>& recoveredInstances,
                                             const std::vector<uint64_t>& timeoutInstances, bool& shouldStopPolling);
    ...
    std::string GetErrorLocationStrFromFaultMsg(const fault::FaultMsgSignal& faultMsg, uint64_t instanceId,
                                                const std::string& targetErrCode);
    ...
    void ProcessRoCERecovery(uint64_t instanceId, const std::vector<FaultNodeInfo>& faultNodes,
    ...
    void ProcessPrefillInstanceFaults(uint64_t instanceId, const std::vector<FaultNodeInfo>& faultNodes,
                                      const std::vector<uint64_t>& instanceNodeIds);
    void ProcessFullNPURecovery(uint64_t instanceId, const std::vector<FaultNodeInfo>& faultNodes,
                                const std::vector<uint64_t>& instanceNodeIds,
                                const std::unordered_set<std::string>& instancePodIPs);
    ...
    bool SendNodeManagerCommandToPodsParallel(const std::unordered_set<std::string>& podIPs, uint64_t instanceId,
                                              NodeManagerCmd cmd);
    ...
    int32_t SaveFaultsWithLock(const std::string& path, const std::vector<std::string>& newFaultsToAdd) const;
};
