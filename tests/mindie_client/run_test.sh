...
function setup_environment() {
    ...
    # Define paths
    DT_MIECLIENT_SRC_HOME="$DT_ROOT_HOME/mindie_service/client"
    DT_CERTUTIL_SRC_HOME="$DT_ROOT_HOME/utils/cert/scripts"
    # Set PYTHONPATH
    export PYTHONPATH="$DT_ROOT_HOME:$DT_MIECLIENT_SRC_HOME:$DT_CERTUTIL_SRC_HOME"
...
function run_tests() {
    ...
    pytest ${TEST_DIR} --cov=mindieclient --cov-branch \
    --junit-xml=${REPORT_PATH}/mindieclient_final.xml --html=${REPORT_PATH}/mindieclient_final.html \
    --self-contained-html \
    --continue-on-collection-errors #避免collecting error中断test session，建议开发环境下取消该参数以提高效率

    coverage html -d ${REPORT_PATH}/report_mindieclient/htmlcov
    coverage xml -o ${REPORT_PATH}/report_mindieclient/coverage.xml
    deactivate
}
