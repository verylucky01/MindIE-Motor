...
public:
    explicit PResInvalidMock(const std::string &keyInit) : MindIEServerMock(keyInit) {}
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        threads.emplace_back(std::thread([predictIP, pPort, manageIP, mPort]() {
            ...
            handler1.RegisterFun(boost::beast::http::verb::post,
                "/v1/chat/completions",
                std::bind(&PResInvalidMock::OpenAIPReqHandler, mock.get(), std::placeholders::_1));
            handler1.RegisterFun(boost::beast::http::verb::get,
                "/dresult",
                std::bind(&PResInvalidMock::DReqHandler, mock.get(), std::placeholders::_1));
            ...
            handler2.RegisterFun(boost::beast::http::verb::post,
                "/v1/role/prefill",
                std::bind(&PResInvalidMock::RolePHandler, mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::post,
                "/v1/role/decode",
                std::bind(&PResInvalidMock::RoleDHandler, mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::get,
                "/v1/config",
                std::bind(&PResInvalidMock::ConfigHandler, mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::get,
                "/v1/status",
                std::bind(&PResInvalidMock::StatusHandler, mock.get(), std::placeholders::_1));
