...
public:
    explicit DResErrorMock(const std::string &keyInit) : MindIEServerMock(keyInit) {}
    ~DResErrorMock() = default;
    void DMessageHandler(std::string reqId, std::string message, bool isStream)
    {
        ServerRes res;
        res.contentType = "text/event-stream";
        res.body += "reqId:" + reqId + '\0' + "error: DResErrorMock return error" + '\0';
        res.isFinish = false;
        dConn->SendRes(res);
    }
};
...
TEST_F(TestDResError, TestDResErrorTC01)
{
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        threads.emplace_back(std::thread([predictIP, pPort, manageIP, mPort] {
            ...
            handler1.RegisterFun(boost::beast::http::verb::post,
                "/v1/chat/completions",
                std::bind(&DResErrorMock::OpenAIPReqHandler, mock.get(), std::placeholders::_1));
            handler1.RegisterFun(boost::beast::http::verb::get,
                "/dresult",
                std::bind(&DResErrorMock::DReqHandler, mock.get(), std::placeholders::_1));
            ...
            handler2.RegisterFun(boost::beast::http::verb::post,
                "/v1/role/prefill",
                std::bind(&DResErrorMock::RolePHandler, mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::post,
                "/v1/role/decode",
                std::bind(&DResErrorMock::RoleDHandler, mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::get,
                "/v1/config",
                std::bind(&DResErrorMock::ConfigHandler, mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::get,
                "/v1/status",
                std::bind(&DResErrorMock::StatusHandler, mock.get(), std::placeholders::_1));
    ...
    EXPECT_EQ(response, "{\"index\":0,\"value\":\"M\"} DResErrorMock return error");
    EXPECT_EQ(code, 0);
