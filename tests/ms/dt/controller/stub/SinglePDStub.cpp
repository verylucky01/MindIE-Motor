...
static bool g_postRoleReqSuccess = true;
...
static int32_t g_serverFaultyCodeRet = 500;
...
int32_t AddServerMock(const std::string &port, ServerMock mock)
{
    std::lock_guard<std::mutex> guard(g_serverMutex);
    auto iter = g_serverMock.find(port);
    if (iter == g_serverMock.end()) {
        g_serverMock[port] = mock;
    }
}
...
static int32_t DealCoordinatorReq(const std::string &ip, const Request &request, std::string& responseBody,
    int32_t &code)
{
...
static int32_t GetServerConfig(const std::string &port, std::string& responseBody, int32_t &code)
{
    code = g_okCodeRet;
...
static int32_t GetServerStatus(const std::string &port, std::string &responseBody, int32_t &code)
{
    code = g_okCodeRet;
    nlohmann::json jsonResponse;
    {
        ...
        for (auto &id : iter->second.peersId) {
            // mock所有建链均成功
            nlohmann::json tmpPeer;
            tmpPeer["target"] = (uint64_t)std::stoull(id);
            tmpPeer["link"] = "ok";
    ...
    responseBody = jsonResponse.dump();
    std::cout << "server " << port << " status " << responseBody << std::endl;
    return 0;
}

static int32_t PostServerRole(const std::string &port, const Request &request, std::string& responseBody,
    int32_t &code, const std::string &role)
{
    code = g_okCodeRet;
    {
        ...
        iter->second.peersId.clear();
        auto bodyJson = nlohmann::json::parse(request.body);
        auto peers = bodyJson.at("peers");
        {
            for (auto it = peers.begin(); it != peers.end(); ++it) {
                auto connId = std::to_string(it->at("id").template get<uint64_t>());
                iter->second.peersId.emplace_back(connId);
            }
        }
    }
    return 0;
...
static int32_t HttpCommFailed(std::string &responseBody, int32_t &code)
{
    responseBody = "";
    code = g_serverFaultyCodeRet;
    return -1;
}

static int32_t DealServerReq(const std::string &port, const Request &request, std::string &responseBody,
    int32_t &code)
{
    if ((request.target == ControllerConstant::GetInstance()->GetServerURI(ServerURI::GET_CONFIG)) &&
        (request.method == boost::beast::http::verb::get)) {
        std::cout << "get config for server " << port << std::endl;
        return GetServerConfig(port, responseBody, code);
    }

    if ((request.target == ControllerConstant::GetInstance()->GetServerURI(ServerURI::GET_STATUS_V1)) &&
        (request.method == boost::beast::http::verb::get)) {
        std::cout << "get status for server " << port << std::endl;
        // 状态查询请求永远成功
        return GetServerStatus(port, responseBody, code);
    }
    ...
    if ((request.target == ControllerConstant::GetInstance()->GetServerURI(ServerURI::POST_DECODE_ROLE_V1)) &&
        ...
        std::cout << "post role " << role << " for server " << port << std::endl;
        if (g_postRoleReqSuccess) {
            return PostServerRole(port, request, responseBody, code, role);
        } else {
            return HttpCommFailed(responseBody, code);
        }
    }
...
{
    HttpClient *httpClientObj = (HttpClient*)obj;
    if (httpClientObj->mHost == g_coordinatorIp) {
        return DealCoordinatorReq(httpClientObj->mHost, request, responseBody, code);
    }
    // use port to distinguish server instance
    return DealServerReq(httpClientObj->mPort, request, responseBody, code);
}
