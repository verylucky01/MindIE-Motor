...
std::string GetAbsolutePath(const std::string &base,
...
std::string GetMSControllerBinPath() {
    ...
    std::string parentPath =
        GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
    std::string jsonPath = GetAbsolutePath(
        parentPath, "mindie_motor/src/output/bin/ms_controller");
    return jsonPath;
}
...
std::string GetMSCoordinatorBinPath() {
    ...
    std::string parentPath =
        GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
    std::string jsonPath = GetAbsolutePath(
        parentPath, "mindie_motor/src/output/bin/ms_coordinator");
    return jsonPath;
}
...
std::string GetMSControllerConfigJsonPath() {
    ...
    std::string parentPath =
        GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
    std::string jsonPath = GetAbsolutePath(
        parentPath, "mindie_motor/src/config/ms_controller.json");
    return jsonPath;
}

// 返回测试公共配置目录（等价于默认的 tests/ms/common/.mindie_ms）。
// 若环境变量 MINDIE_MS_TEST_CONFIG_BASEDIR 已设置，则直接使用该目录，
// 用于 UT/ST 隔离场景（避免并发时互相覆盖配置文件）。
static std::string GetTestConfigBaseDir() {
    const char *envDir = std::getenv("MINDIE_MS_TEST_CONFIG_BASEDIR");
    ...
    std::string parentPath =
        GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
    return GetAbsolutePath(parentPath, "tests/ms/common/.mindie_ms");
}

static std::string GetControllerTestAssetPath(const std::string &fileName) {
    return GetAbsolutePath(GetTestConfigBaseDir(), fileName);
}

static std::string GetControllerTestLogDir() {
    return GetAbsolutePath(GetTestConfigBaseDir(), "logs");
}

std::string GetMSControllerTestJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(), "ms_controller.json");
}
...
std::string GetServerRequestHandlerTestJsonPath() {
    return GetControllerTestAssetPath("ms_controller_1.json");
}

std::string GetProbeServerTestJsonPath() {
    return GetControllerTestAssetPath("ms_controller_2.json");
}

std::string GetAlarmManagerTestJsonPath() {
    return GetControllerTestAssetPath("ms_controller_3.json");
}
...
std::string GetModelConfigJsonPath() {
    ...
    std::string parentPath =
        GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
    std::string jsonPath = GetAbsolutePath(
        parentPath, "mindie_motor/src/config/model_config/llama2-70B.json");
    return jsonPath;
}
...
std::string GetMachineConfigJsonPath() {
    ...
    std::string parentPath =
        GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
    std::string jsonPath = GetAbsolutePath(
        parentPath, "mindie_motor/src/config/machine_config/800IA2.json");
    return jsonPath;
}

std::string GetModelConfigTestJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(), "llama2-70B.json");
}

std::string GetMachineConfigTestJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(), "800IA2.json");
}

std::string GetMSGlobalRankTableTestJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(), "global_rank_table.json");
}

std::string GetMSRankTableLoaderTestJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(),
                           "global_rank_table_tmp.json");
}

std::string GetCrossNodeMSRankTableLoaderJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(),
                           "global_rank_table_cross_node.json");
}

std::string GetA2CrossNodeMSRankTableLoaderJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(),
                           "global_rank_table_cross_node_a2.json");
}

std::string GetElasticScalingJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(), "elastic_scaling.json");
}

std::string GetA3CrossNodeMSRankTableLoaderJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(),
                           "global_rank_table_cross_node_a3.json");
}
...
std::string GetMSDeployConfigJsonPath() {
    ...
    std::string parentPath =
        GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
    std::string jsonPath = GetAbsolutePath(
        parentPath, "mindie_motor/src/config/infer_server.json");
    return jsonPath;
}

std::string GetMSDeployTestJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(), "infer_server.json");
}
...
std::string GetMSCoordinatorConfigJsonPath() {
    ...
    std::string parentPath =
        GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
    std::string jsonPath = GetAbsolutePath(
        parentPath, "mindie_motor/src/config/ms_coordinator.json");
    return jsonPath;
}

std::string GetMSCoordinatorTestJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(), "ms_coordinator.json");
}
...
std::string GetMSCoordinatorRunLogPath() {
    ...
    std::string parentPath =
        GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
    std::string jsonPath =
        GetAbsolutePath(parentPath, "tests/ms/dt/logs/coordinator_run_log.log");
    return jsonPath;
}
...
std::string GetMSCoordinatorOperationLogPath() {
    ...
    std::string parentPath =
        GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
    std::string jsonPath = GetAbsolutePath(
        parentPath, "tests/ms/dt/logs/coordinator_operation_path.log");
    return jsonPath;
}

std::string GetMSControllerStatusJsonTestJsonPath() {
    return GetAbsolutePath(GetTestConfigBaseDir(), "status_json.json");
}
...
bool RemoveDirectoryRecursively(const std::string &path) {
    ...
    while ((entry = readdir(dir)) != nullptr) {
        ...
        if (stat(entryPath.c_str(), &statbuf) == -1) {
            std::cerr << "Failed to stat entry: " << strerror(errno)
                      << std::endl;
            closedir(dir);
            return false;
        }
        ...
        if (S_ISDIR(statbuf.st_mode)) {
            ...
            if (remove(entryPath.c_str()) != 0) {
                std::cerr << "Failed to remove file: " << strerror(errno)
                          << std::endl;
                closedir(dir);
                return false;
            }
...
std::string SetMSServerJsonDefault(const std::string &jsonPath) {
    ...
    manager.SetList({"log_info", "operation_log_path"},
                    "./log/operation_log.txt");
    ...
    auto lambdaFun = [exeDir, &manager](std::string items) mutable {
        auto ca_cert =
            JoinPathComponents({exeDir, "cert_dir", items + "_ca_cert.crt"});
        CreateFile(ca_cert, "ca_cert");
        ChangeCertsFileMode(ca_cert, S_IRUSR);

        auto tls_cert =
            JoinPathComponents({exeDir, "cert_dir", items + "_tls_cert.crt"});
        CreateFile(tls_cert, "tls_cert");
        ChangeCertsFileMode(tls_cert, S_IRUSR);

        auto tls_key =
            JoinPathComponents({exeDir, "cert_dir", items + "_tls_key.key"});
        CreateFile(tls_key, "tls_key");
        ChangeCertsFileMode(tls_key, S_IRUSR);

        auto tls_crl =
            JoinPathComponents({exeDir, "cert_dir", items + "_tls_crl.crl"});
...
std::string SetMSCoordinatorJsonDefault(const std::string &jsonPath) {
    ...
    manager.SetList({"log_info", "operation_log_path"},
                    "./log/operation/log.txt");
    manager.SetList({"log_info", "max_log_str_size"}, 4096);

    manager.SetList({"digs_scheduler_config", "deploy_mode"}, "pd_separate");
    manager.SetList({"digs_scheduler_config", "scheduler_type"},
                    "digs_scheduler");
    manager.SetList({"digs_scheduler_config", "algorithm_type"},
                    "load_balance");
    ...
    manager.SetList({"digs_scheduler_config", "res_view_update_timeout"},
                    "500");
    manager.SetList({"digs_scheduler_config", "metaResource_names"}, "");
    manager.SetList({"digs_scheduler_config", "load_cost_values"}, "1, 0");
    manager.SetList({"digs_scheduler_config", "load_cost_coefficient"},
                    "0, 1, 1024, 24, 6, 0, 0, 1, 1");
    manager.SetList({"digs_scheduler_config", "tokenizer_weight_path"},
                    "/data/atb_testdata/weights/llama1-7b");
...
std::string SetMSControllerJsonDefault(const std::string &jsonPath) {
    std::string logDir = GetControllerTestLogDir();
    struct stat logDirInfo;
    if (stat(logDir.c_str(), &logDirInfo) == 0 &&
        (logDirInfo.st_mode & S_IFDIR)) {
        RemoveDirectoryRecursively(logDir);
    }
    CreateDirectory(logDir);
    JsonFileManager manager(jsonPath);
    ...
    manager.Set("role_decision_time_period",
                3600);  // 身份切换的时间间隔为3600秒
    ...
    std::string exeDir = GetParentPath(GetExecutablePath());
    std::string binaryLogDir = JoinPathComponents({exeDir, "log"});
    CreateDirectory(binaryLogDir);
    manager.SetList({"log_info", "log_level"}, "DEBUG");
    manager.SetList({"log_info", "run_log_path"},
                    JoinPathComponents({logDir, "run_log.txt"}));
    manager.SetList({"log_info", "operation_log_path"},
                    JoinPathComponents({logDir, "operation_log.txt"}));
    manager.SetList(
        {"process_manager", "file_path"},
        JoinPathComponents({logDir, "controller_process_status.json"}));
    manager.SetList({"cluster_status", "file_path"},
                    JoinPathComponents({logDir, "cluster_status_output.json"}));
...
std::string SetMSClientJsonDefault(const std::string &jsonPath) {
    ...
    auto ca_cert =
        JoinPathComponents({exeDir, "cert_dir", "msctl_ca_cert.crt"});
    CreateFile(ca_cert, "ca_cert");
    ChangeCertsFileMode(ca_cert, S_IRUSR);

    auto tls_cert =
        JoinPathComponents({exeDir, "cert_dir", "msctl_tls_cert.crt"});
    CreateFile(tls_cert, "tls_cert");
    ChangeCertsFileMode(tls_cert, S_IRUSR);

    auto tls_key =
        JoinPathComponents({exeDir, "cert_dir", "msctl_tls_key.key"});
    CreateFile(tls_key, "tls_key");
    ChangeCertsFileMode(tls_key, S_IRUSR);

    auto tls_crl =
        JoinPathComponents({exeDir, "cert_dir", "msctl_tls_crl.crl"});
    CreateFile(tls_crl, "tls_crl");
    ChangeCertsFileMode(tls_crl, S_IRUSR);
...
std::string SetInferJsonDefault(const std::string &jsonPath) {
    ...
    manager.SetList({"resource_requests", "cpu_core"},
                    32000);  // 32000 cpu_core
    manager.SetList({"resource_requests", "npu_type"}, "Ascend910");
    manager.SetList({"resource_requests", "npu_chip_num"}, 8);  // 8 npu核心数
    manager.SetList({"mindie_server_config", "infer_port"},
                    1025);  // 1025 infer端口
    manager.SetList({"mindie_server_config", "management_port"},
                    1026);  // 1026 management_port
...
bool CopyFile(const std::string &sourcePath, const std::string &destPath) {
    ...
    if (std::remove(destPath.c_str()) != 0) {
        std::cerr << "Error removing existing destination file: " << destPath
                  << std::endl;
    }
    ...
    if (!destFile.is_open()) {
        std::cerr << "Error creating destination file: " << destPath
                  << std::endl;
        sourceFile.close();
        return false;
    }
...
std::string GetHSECEASYPATH() {
    ...
    if (thirdPartyDir == nullptr) {
        ...
        parentPath =
            GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
        jsonPath = GetAbsolutePath(
            parentPath, "mindie_motor/src/build/open_source/kmc/lib");
    } else {
...
bool WaitUtilTrue(WaitFunc func, uint32_t timeoutSeconds,
    ...
    while (true) {
        ...
        usleep(intervalMilliSeconds *
               1000);  // 休眠时间，默认100毫秒，外部可配置
        auto passTime =
            std::chrono::duration_cast<seconds>(currentTime - startTime)
                .count();
...
bool FindStringInLogFile(const std::string &filename,
...
std::string GetLdLibraryPath() {
    const char *thirdPartyDir = std::getenv("BUILD_MIES_3RDPARTY_INSTALL_DIR");
    const char *miesInstallDir =
        std::getenv("BUILD_MINDIE_SERVICE_INSTALL_DIR");
    std::string parentPath = "";
    std::string jsonPath = "";
    ...
    if (thirdPartyDir != nullptr && miesInstallDir != nullptr) {
        ...
        jsonPath +=
            ":" + GetAbsolutePath(parentPath,
                                  "mindie_motor/src/build/open_source/openssl");
        jsonPath +=
            ":" + GetAbsolutePath(parentPath,
                                  "mindie_motor/src/build/open_source/kmc/lib");
        jsonPath +=
            ":" + GetAbsolutePath(
                      parentPath,
