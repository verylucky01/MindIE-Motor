...
class MindIEServerMock {
   public:
    ...
    virtual void SetPDRespond(std::shared_ptr<ServerConnection> connection,
                              bool isStream, std::string question,
                              nlohmann::json &pResJson);
    virtual void TritonPReqHandler(
        std::shared_ptr<ServerConnection> connection);
    virtual void TGIPReqHandler(std::shared_ptr<ServerConnection> connection);
    virtual void MindIEPReqHandler(
        std::shared_ptr<ServerConnection> connection);
    virtual void OpenAIPReqHandler(
        std::shared_ptr<ServerConnection> connection);
    virtual void DReqHandler(std::shared_ptr<ServerConnection> connection);
    virtual void DMessageHandler(std::string reqId, std::string message,
                                 bool isStream);
    ...
    virtual void OpenAISingleReqHandler(
        std::shared_ptr<ServerConnection> connection);
    virtual void TritonSingleReqHandler(
        std::shared_ptr<ServerConnection> connection);
...
class ClusterMock {
   public:
    void AddInstance(const std::string ip, const std::string port,
                     std::shared_ptr<MindIEServerMock> instance) {
        std::string key = ip + ":" + port;
        std::unique_lock<std::mutex> lock(mtx);
        cluster[key] = instance;
    }
    std::shared_ptr<MindIEServerMock> GetInstance(const std::string ip,
...
void CreateSingleServer(const std::string &ip1, const std::string &port1,
                        ...
                        uint32_t threadNum = 4, int32_t serverId = 0,
