...
MindIEServerMock::MindIEServerMock(const std::string &keyInit)
    : key(keyInit), role("none"), init(false) {}
...
void MindIEServerMock::InitMetrics() {
    ...
    std::string metricsSingleNodeData0File = GetAbsolutePath(
        parentPath,
        "tests/ms/common/.mindie_ms/ms_metricsSingleNodeData_0.bin");
    std::string metricsSingleNodeData1File = GetAbsolutePath(
        parentPath,
        "tests/ms/common/.mindie_ms/ms_metricsSingleNodeData_1.bin");
    std::string metricsSingleNodeData2File = GetAbsolutePath(
        parentPath,
        "tests/ms/common/.mindie_ms/ms_metricsSingleNodeData_2.bin");
    std::string metricsSingleNodeData3File = GetAbsolutePath(
        parentPath,
        "tests/ms/common/.mindie_ms/ms_metricsSingleNodeData_3.bin");
    std::vector<std::string> singleFilePath = {
        metricsSingleNodeData0File, metricsSingleNodeData1File,
    ...
    std::vector<std::string> PDFilePath = {
        metricsPDNodeData0File, metricsPDNodeData1File, metricsPDNodeData2File,
        metricsPDNodeData3File};
...
std::string MindIEServerMock::GetKey() const { return key; }
void MindIEServerMock::SetServerId(int32_t serverId) { id = serverId; }

void MindIEServerMock::SetPDRespond(
    std::shared_ptr<ServerConnection> connection, bool isStream,
    std::string question, nlohmann::json &pResJson) {
    ...
    delimiterIndex =
        (delimiterIndex == std::string::npos) ? dTarget.size() : delimiterIndex;
    ...
    if (dInstance != nullptr) {
        ...
        MyThreadPool::GetInstance().submit(&MindIEServerMock::DMessageHandler,
                                           dInstance.get(), reqId, question,
                                           isStream);
    }
...
// P发送响应函数
void MindIEServerMock::TritonPReqHandler(
    ...
    try {
        ...
        if (req.target().ends_with("generate_stream")) {
            ...
            for (auto it = inputs.begin(); it != inputs.end(); ++it) {
                auto data = it->at("data");
                auto data_ = data.get<std::vector<uint32_t>>();
                for (auto dd : data_) {
...
// P发送响应函数
void MindIEServerMock::TGIPReqHandler(
    std::shared_ptr<ServerConnection> connection) {
    auto &req = connection->GetReq();
    std::string reqId = req["req-id"];
    auto dTarget = req["d-target"];
...
// P发送响应函数
void MindIEServerMock::MindIEPReqHandler(
    std::shared_ptr<ServerConnection> connection) {
    auto &req = connection->GetReq();
    std::string reqId = req["req-id"];
    auto dTarget = req["d-target"];
...
// P发送响应函数
void MindIEServerMock::OpenAIPReqHandler(
    std::shared_ptr<ServerConnection> connection) {
    auto &req = connection->GetReq();
    std::string reqId = req["req-id"];
    auto dTarget = req["d-target"];
...
// D收到请求的响应函数
void MindIEServerMock::DReqHandler(
    std::shared_ptr<ServerConnection> connection) {
...
// D发送响应函数
void MindIEServerMock::DMessageHandler(std::string reqId, std::string message,
                                       bool isStream) {
    ...
    if (isStream) {
        for (size_t i = 0; i < message.size(); ++i) {
            ...
            if (i == (message.size() - 1)) {
                res.body +=
                    "reqId:" + reqId + '\0' + "lastData:" + data.dump() + '\0';
            } else {
                res.body =
                    "reqId:" + reqId + '\0' + "data:" + data.dump() + '\0';
            }
            res.contentType = "text/event-stream";
            res.isFinish = false;
            dConn->SendRes(res);
            // std::cout << "D res: " << reqId << ", data: " << res.body <<
            // std::endl;
        }
    } else {
        ServerRes res;
...
void MindIEServerMock::RolePHandler(
    std::shared_ptr<ServerConnection> connection) {
...
void MindIEServerMock::RoleDHandler(
    std::shared_ptr<ServerConnection> connection) {
...
void MindIEServerMock::ConfigHandler(
    std::shared_ptr<ServerConnection> connection) {
    nlohmann::json resJson;
...
void MindIEServerMock::StatusHandler(
    std::shared_ptr<ServerConnection> connection) {
    nlohmann::json resJson;
...
// 监控指标查询响应函数 /metrics
void MindIEServerMock::MetricsHandler(
    std::shared_ptr<ServerConnection> connection) {
    ServerRes res;
...
void MindIEServerMock::PDMetricsHandler(
    std::shared_ptr<ServerConnection> connection) {
    ServerRes res;
...
void MindIEServerMock::TokenizerHandler(
    std::shared_ptr<ServerConnection> connection) {
    auto &req = connection->GetReq();
    auto body = boost::beast::buffers_to_string(req.body().data());
    std::string question = "";
...
void MindIEServerMock::OpenAISingleReqHandler(
    std::shared_ptr<ServerConnection> connection) {
    auto &req = connection->GetReq();
    std::string reqId = req["req-id"];
    bool stream = false;
...
void MindIEServerMock::TritonSingleReqHandler(
    std::shared_ptr<ServerConnection> connection) {
    auto &req = connection->GetReq();
    std::string reqId = req["req-id"];
    bool stream = false;
...
void CreateSingleServer(const std::string &ip1, const std::string &port1,
                        ...
                        uint32_t threadNum, int32_t serverId,
    ...
    handler1.RegisterFun(boost::beast::http::verb::post, "/v1/chat/completions",
                         std::bind(&MindIEServerMock::OpenAISingleReqHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    handler1.RegisterFun(boost::beast::http::verb::post,
                         "/v2/models/*/generate_stream",
                         std::bind(&MindIEServerMock::TritonSingleReqHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    ...
    handler1.RegisterFun(boost::beast::http::verb::post, "/v2/models/*/infer",
                         std::bind(&MindIEServerMock::TritonSingleReqHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    handler1.RegisterFun(boost::beast::http::verb::post, "/v1/tokenizer",
                         std::bind(&MindIEServerMock::TokenizerHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    ...
    handler2.RegisterFun(boost::beast::http::verb::post, "/v1/role/prefill",
                         std::bind(&MindIEServerMock::RolePHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    handler2.RegisterFun(boost::beast::http::verb::post, "/v1/role/decode",
                         std::bind(&MindIEServerMock::RoleDHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    handler2.RegisterFun(boost::beast::http::verb::get, "/v1/config",
                         std::bind(&MindIEServerMock::ConfigHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    handler2.RegisterFun(boost::beast::http::verb::get, "/v1/status",
                         std::bind(&MindIEServerMock::StatusHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    ...
    handler3.RegisterFun(boost::beast::http::verb::get, "/metrics",
                         std::bind(&MindIEServerMock::MetricsHandler,
                                   openAIMock1.get(), std::placeholders::_1));
...
void CreateMindIEServer(const std::string &ip1, const std::string &port1,
    ...
    handler1.RegisterFun(boost::beast::http::verb::post,
                         "/v2/models/*/generate_stream",
                         std::bind(&MindIEServerMock::TritonPReqHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    ...
    handler1.RegisterFun(boost::beast::http::verb::post, "/",
                         std::bind(&MindIEServerMock::TGIPReqHandler,
                                   openAIMock1.get(), std::placeholders::_1));

    handler1.RegisterFun(boost::beast::http::verb::post, "/generate_stream",
                         std::bind(&MindIEServerMock::TGIPReqHandler,
                                   openAIMock1.get(), std::placeholders::_1));

    handler1.RegisterFun(boost::beast::http::verb::post, "/generate",
                         std::bind(&MindIEServerMock::TGIPReqHandler,
                                   openAIMock1.get(), std::placeholders::_1));

    handler1.RegisterFun(boost::beast::http::verb::post, "/infer",
                         std::bind(&MindIEServerMock::MindIEPReqHandler,
                                   openAIMock1.get(), std::placeholders::_1));

    handler1.RegisterFun(boost::beast::http::verb::post, "/v1/completions",
                         std::bind(&MindIEServerMock::OpenAIPReqHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    ...
    handler1.RegisterFun(boost::beast::http::verb::get, "/dresult",
                         std::bind(&MindIEServerMock::DReqHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    handler1.RegisterFun(boost::beast::http::verb::post, "/v1/tokenizer",
                         std::bind(&MindIEServerMock::TokenizerHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    ...
    ServerHandler handler3;
    handler3.RegisterFun(boost::beast::http::verb::get, "/metrics",
                         std::bind(&MindIEServerMock::PDMetricsHandler,
                                   openAIMock1.get(), std::placeholders::_1));
    HttpServerParm parm3;
    parm3.address = ip2;
    parm3.port = metricPort;  // metric端口号
...
void CreateSSLServer(const std::string &ip1, const std::string &port1,
                     const std::string &ip2, const std::string &port2,
                     uint32_t threadNum, const std::string &metricPort) {
    std::string exePath = GetExecutablePath();
    std::string parentPath =
        GetParentPath(GetParentPath(GetParentPath(GetParentPath(exePath))));
    std::string certDir =
        GetAbsolutePath(parentPath, "tests/ms/dt/coordinator/cert_files");
    TlsItems serverTlsItems;
    serverTlsItems.caCert =
        GetAbsolutePath(certDir, "mindie_server_to_coordinator/ca.pem");
    serverTlsItems.tlsCert =
        GetAbsolutePath(certDir, "mindie_server_to_coordinator/cert.pem");
    serverTlsItems.tlsKey =
        GetAbsolutePath(certDir, "mindie_server_to_coordinator/cert.key.pem");
    serverTlsItems.tlsPasswd = GetAbsolutePath(
        certDir, "mindie_server_to_coordinator/cert_passwd.txt");
    HttpServer httpServer1;
    httpServer1.Init(threadNum);
    // 设置回调函数
    ServerHandler handler1;
    ...
    TlsItems tlsItems;
    tlsItems.caCert =
        GetAbsolutePath(certDir, "mindie_server_manage_to_coordinator/ca.pem");
    tlsItems.tlsCert = GetAbsolutePath(
        certDir, "mindie_server_manage_to_coordinator/cert.pem");
    tlsItems.tlsKey = GetAbsolutePath(
        certDir, "mindie_server_manage_to_coordinator/cert.key.pem");
    tlsItems.tlsPasswd = GetAbsolutePath(
        certDir, "mindie_server_manage_to_coordinator/cert_passwd.txt");
    ServerHandler handler3;
    handler3.RegisterFun(boost::beast::http::verb::get, "/metrics",
                         std::bind(&MindIEServerMock::PDMetricsHandler,
                                   openAIMock1.get(), std::placeholders::_1));
...
void CreateMetricsSSLServer(const std::string &ip1, const std::string &port1,
    ...
    serverTlsItems.caCert =
        GetAbsolutePath(certDir, "mindie_server_manage_to_coordinator/ca.pem");
    serverTlsItems.tlsCert = GetAbsolutePath(
        certDir, "mindie_server_manage_to_coordinator/cert.pem");
    serverTlsItems.tlsKey = GetAbsolutePath(
        certDir, "mindie_server_manage_to_coordinator/cert.key.pem");
    serverTlsItems.tlsPasswd = GetAbsolutePath(
        certDir, "mindie_server_manage_to_coordinator/cert_passwd.txt");
    ServerHandler handler2;
    handler2.RegisterFun(boost::beast::http::verb::get, "/metrics",
                         std::bind(&MindIEServerMock::PDMetricsHandler,
                                   mock.get(), std::placeholders::_1));
    HttpServerParm parm2;
    parm2.address = ip2;
