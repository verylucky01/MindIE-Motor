...
int32_t SendInferRequest(std::string dstIp, std::string dstPort,
                         MINDIE::MS::TlsItems tlsItems, std::string url,
                         std::string reqBody, std::string &response) {
...
int32_t SendSSLInferRequest(std::string dstIp, std::string dstPort,
                            MINDIE::MS::TlsItems tlsItems, std::string url,
                            std::string reqBody, std::string &response) {
    std::cout << "reqBody: " << reqBody << std::endl;
    HttpClient httpClient;
...
int32_t GetInferRequest(std::string dstIp, std::string dstPort,
                        MINDIE::MS::TlsItems tlsItems, std::string url,
                        std::string &response, bool tls) {
    HttpClient httpClient;
...
int32_t GetMetricsRequest(std::string dstIp, std::string dstPort,
                          MINDIE::MS::TlsItems tlsItems, std::string url,
                          std::string &response, bool tls) {
    HttpClient httpClient;
    tlsItems.tlsEnable = tls;
...
bool SetSingleRole(ThreadSafeVector<std::string> &predictIPs,
                   ThreadSafeVector<std::string> &predictPorts,
                   ThreadSafeVector<std::string> &managePortLists,
    ...
    auto lambdaFunc = [manageIP, managePort, instancesInfo]() -> bool {
        TlsItems tlsItems;
        std::string response;
        auto ret = SendInferRequest(manageIP, managePort, tlsItems,
...
nlohmann::json SetPDRoleByList(
    ThreadSafeVector<std::string> &predictIPs,
    ThreadSafeVector<std::string> &predictPorts,
    ThreadSafeVector<std::string> &managePortLists,
    ThreadSafeVector<std::string> &interCommonPortLists, uint8_t numPRole,
    uint8_t numDRole) {
    nlohmann::json instancesInfo;
    instancesInfo["ids"] = nlohmann::json::array();
    instancesInfo["instances"] = nlohmann::json::array();
...
bool SetPDRole(ThreadSafeVector<std::string> &predictIPs,
               ThreadSafeVector<std::string> &predictPorts,
               ThreadSafeVector<std::string> &managePortLists,
               ThreadSafeVector<std::string> &interCommonPortLists,
               uint8_t numPRole, uint8_t numDRole, std::string &manageIP,
               std::string &managePort) {
    auto pdInstanceInfo =
        SetPDRoleByList(predictIPs, predictPorts, managePortLists,
...
bool SetPDRoleSSL(ThreadSafeVector<std::string> &predictIPs,
                  ThreadSafeVector<std::string> &predictPorts,
                  ThreadSafeVector<std::string> &managePortLists,
                  ThreadSafeVector<std::string> &interCommonPortLists,
                  uint8_t numPRole, uint8_t numDRole, std::string &manageIP,
                  std::string &managePort, MINDIE::MS::TlsItems tlsItems) {
    auto pdInstanceInfo =
        SetPDRoleByList(predictIPs, predictPorts, managePortLists,
                        interCommonPortLists, numPRole, numDRole);

    auto lambdaFunc = [manageIP, managePort, tlsItems,
                       pdInstanceInfo]() -> bool {
        std::string response;
        auto ret = SendSSLInferRequest(manageIP, managePort, tlsItems,
                                       "/v1/instances/refresh",
                                       pdInstanceInfo.dump(), response);
        std::cout << "------------ SetPDRoleSSL ret ----------------" << ret
                  << std::endl;
        if (ret == 0) {
            std::cout << "------------ SetPDRoleSSL Ready ----------------"
                      << std::endl;
            return true;
        } else {
            return false;
...
int32_t WaitCoordinatorReady(std::string &manageIP, std::string &managePort) {
    auto lambdaFunc = [manageIP, managePort]() -> bool {
        TlsItems tlsItems;
        std::string response;
        auto ret = GetInferRequest(manageIP, managePort, tlsItems,
...
int32_t WaitCoordinatorReadySSL(std::string &manageIP, std::string &managePort,
                                TlsItems tlsItems) {
    auto lambdaFunc = [manageIP, managePort, tlsItems]() -> bool {
        std::string response;
        auto ret = GetInferRequest(manageIP, managePort, tlsItems,
                                   "/v2/health/ready", response, true);
        if (ret == 200) {
            std::cout << "------------ Coordinator Ready ----------------"
                      << std::endl;
            return true;
...
bool Compare(const Item &a, const Item &b) { return a.index < b.index; }
...
bool AddReqMock(
    boost::beast::string_view reqId, ReqInferType type,
    std::shared_ptr<ServerConnection> connection,
    boost::beast::http::request<boost::beast::http::dynamic_body> &req) {
    return false;
}

size_t GetReqNumMock() { return 4294967290; }
...
int32_t LinkWithDNodeInMaxRetryMock(const std::string &ip,
                                    const std::string &port, uint64_t insId) {
    (void)insId;
...
std::string CreateOpenAIExceptionRequest(std::vector<std::string> contents,
                                         bool stream) {
    // OpenAI request
    nlohmann::json request;
    request["model"] = "llama2_7b";
...
std::string CreateOpenAICompletionsExceptionRequest(std::string content,
                                                    bool stream) {
    // OpenAI request
    nlohmann::json request;
    request["model"] = "llama2_7b";
