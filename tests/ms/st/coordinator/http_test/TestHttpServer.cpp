...
class TestHttp : public testing::Test {
   protected:
    static std::size_t run_stub() { std::cout << "run_stub" << std::endl; }
};
...
TEST_F(TestHttp, UTHttpServerConcurrencyTC02) {
    ...
    parm2.address =
        Configure::Singleton()->httpConfig.managementIp;  // 管理面ip
    parm2.port =
        Configure::Singleton()->httpConfig.managementPort;  // 管理面端口
