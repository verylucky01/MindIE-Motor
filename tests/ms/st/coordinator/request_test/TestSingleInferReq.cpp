...
class TestSingleInferReq : public testing::Test {
   protected:
    void SetUp() override {
        CopyDefaultConfig();
...
class TestSingleInferReqPrompt
    : public TestSingleInferReq,
      public ::testing::WithParamInterface<PromptParam> {
    void SetUp() { TestSingleInferReq::SetUp(); }
};
...
/*
测试描述: 使用单机模式，发送不同协议的推理请求
测试步骤:
    1. 配置单机模式，发送类型错误和缺失prompt的请求，预期报错
    2. 预期结果1
预期结果:
    1. 报错，response为"Request format is invalid\r\n"
*/
TEST_P(TestSingleInferReqPrompt, TestSingleInferReqTC) {
    auto coordinatorJson = GetMSCoordinatorTestJsonPath();
    JsonFileManager manager(coordinatorJson);
    manager.Load();
    ...
    auto code = SendInferRequest(predictIP, predictPort, tlsItems, "/generate",
...
INSTANTIATE_TEST_SUITE_P(, TestSingleInferReqPrompt,
