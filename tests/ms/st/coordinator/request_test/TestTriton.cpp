...
std::string CreateTritonRequest(std::string inferId, std::string textInput,
                                bool separateApi) {
    ...
    if (separateApi) {
        ...
        request["parameters"] = {
            ...
            {"seed", 'null'},       {"repetition_penalty", 1.03},
            {"max_new_tokens", 20}, {"watermark", true}};
    }

    return request.dump();
}

std::string CreateTritonErrRequest(std::string inferId, std::string textInput,
                                   int errCode) {
    nlohmann::json request;
    ...
    if (errCode == 1) {
        ...
        request["inputs"] = {
            {{"name", "input0"}, {"shape", {1, 10}}, {"datatype", "UINT32"}}};
    } else {
        request["inputs"] = {{{"name", "input0"},
                              {"shape", {1, 10}},
                              {"datatype", "UINT32"},
...
TEST_F(TestTriton, TestTritonTC01) {
    ...
    auto nonStreamBody =
        CreateTritonRequest("1", "My name is Olivier and I", true);
    auto streamBody =
        CreateTritonRequest("1", "My name is Olivier and I", true);
    auto singlestreamBody = CreateTritonRequest("1", "M", true);
    auto singleNonstreamBody = CreateTritonRequest("1", "M", true);
    auto nonSeparateBody =
        CreateTritonRequest("1", "My name is Olivier and I", false);

    auto errorBody1 =
        CreateTritonErrRequest("1", "My name is Olivier and I", 1);
    auto errorBody2 =
        CreateTritonErrRequest("1", "My name is Olivier and I", 2);
    auto errorBody3 =
        CreateTritonErrRequest("1", "My name is Olivier and I", 3);
    auto errorBody4 =
        CreateTritonErrRequest("1", "My name is Olivier and I", 4);
    ...
    auto code = SendInferRequest(predictIP, predictPort, tlsItems,
                                 "/v2/models/llama_65b/generate", nonStreamBody,
                                 response);
    EXPECT_EQ(response,
    ...
    code = SendInferRequest(predictIP, predictPort, tlsItems,
                            "/v2/models/llama_65b/generate_stream", streamBody,
                            response);
    ...
    code = SendInferRequest(predictIP, predictPort, tlsItems,
                            "/v2/models/llama_65b/generate_stream",
    ...
    code = SendInferRequest(predictIP, predictPort, tlsItems,
                            "/v2/models/llama_65b/generate",
                            singleNonstreamBody, response);
    EXPECT_EQ(response, "M");  // mock实现为将所有的msg返回
    EXPECT_EQ(code, 0);

    code = SendInferRequest(predictIP, predictPort, tlsItems,
                            "/v2/models/llama_65b/infer", nonSeparateBody,
                            response);
    ...
    // 错误输入1
    code = SendInferRequest(predictIP, predictPort, tlsItems,
                            "/v2/models/llama_65b/infer", errorBody1, response);
    EXPECT_EQ(response,
    ...
    // 错误输入2
    code = SendInferRequest(predictIP, predictPort, tlsItems,
                            "/v2/models/llama_65b/infer", errorBody2, response);
    EXPECT_EQ(response, "396319");  // mock实现为将所有的msg返回
    EXPECT_EQ(code, 0);

    // 错误输入3
    code = SendInferRequest(predictIP, predictPort, tlsItems,
                            "/v2/models/llama_65b/infer", errorBody3, response);
    EXPECT_EQ(response, "396319");  // mock实现为将所有的msg返回
    EXPECT_EQ(code, 0);

    // 错误输入4
    code = SendInferRequest(predictIP, predictPort, tlsItems,
                            "/v2/models/llama_65b/infer", errorBody4, response);
    EXPECT_EQ(response,
              "Request format is invalid\r\n");  // mock实现为将所有的msg返回
    EXPECT_EQ(code, 0);
    ...
    code =
        SendInferRequest(predictIP, predictPort, tlsItems,
                         "/v2/models//generate", singleNonstreamBody, response);
    EXPECT_EQ(response,
              "Request format is invalid\r\n");  // mock实现为将所有的msg返回
    EXPECT_EQ(code, 0);
