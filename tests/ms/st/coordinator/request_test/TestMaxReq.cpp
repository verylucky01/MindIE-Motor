...
class MaxReqMock : public MindIEServerMock {
   public:
    explicit MaxReqMock(const std::string &keyInit)
    ...
    void SetPDRespond(std::shared_ptr<ServerConnection> connection,
        ...
        delimiterIndex = (delimiterIndex == std::string::npos) ? dTarget.size()
                                                               : delimiterIndex;
...
TEST_F(TestMaxReq, TestMaxReqTC01) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        threads.emplace_back(std::thread([predictIP, pPort, manageIP, mPort] {
            ...
            handler1.RegisterFun(
                ...
                std::bind(&MaxReqMock::OpenAIPReqHandler, retryMock.get(),
                          std::placeholders::_1));
            handler1.RegisterFun(
                boost::beast::http::verb::get, "/dresult",
                std::bind(&MaxReqMock::DReqHandler, retryMock.get(),
                          std::placeholders::_1));
            ...
            handler2.RegisterFun(
                ...
                std::bind(&MaxReqMock::RolePHandler, retryMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::post, "/v1/role/decode",
                std::bind(&MaxReqMock::RoleDHandler, retryMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::get, "/v1/config",
                std::bind(&MaxReqMock::ConfigHandler, retryMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::get, "/v1/status",
                std::bind(&MaxReqMock::StatusHandler, retryMock.get(),
                          std::placeholders::_1));
    ...
    auto code = SendInferRequest(predictIP, predictPort, tlsItems,
                                 "/v1/chat/completions", streamBody, response);
    EXPECT_EQ(response, "");  // maxconn = maxreqlimit, so exceeding this will
