...
class TestMetricsPDSeparate : public testing::Test {
   protected:
    void SetUp() {
        ...
        std::string aggregate4PDNodeOutput =
            GetAbsolutePath(parentPath,
                            "tests/ms/common/.mindie_ms/"
                            "ms_metricsAggregate4PDSeparateOutput.bin");
        ...
        manager.SetList(
            ...
            GetAbsolutePath(certDir,
                            "coordinator_manage_to_user/cert.key.pem"));
        manager.SetList(
            {"tls_config", "controller_server_tls_items", "tls_passwd"},
            GetAbsolutePath(certDir,
                            "coordinator_manage_to_user/cert_passwd.txt"));
        ...
        ChangeCertsFileMode(
            GetAbsolutePath(certDir, "user_to_coordinator_manage/ca.pem"),
            S_IRUSR);
        ChangeCertsFileMode(
            GetAbsolutePath(certDir, "user_to_coordinator_manage/cert.pem"),
            S_IRUSR);
        ChangeCertsFileMode(
            GetAbsolutePath(certDir, "user_to_coordinator_manage/cert.key.pem"),
            S_IRUSR);
        ChangeCertsFileMode(
            GetAbsolutePath(certDir,
                            "user_to_coordinator_manage/cert_passwd.txt"),
            S_IRUSR);
...
TEST_F(TestMetricsPDSeparate, TestPDSeparateMetricsTC01) {
    ...
    for (size_t i = 0; i < numMindIEServer; i++) {
        ...
        std::cout << "ip = " << predictIPList[i] << ", predict port = " << pPort
                  << ", manage port = " << mPort
                  << ", metric port = " << metricPort << std::endl;
        threads.push_back(
            std::thread(CreateMindIEServer, std::move(predictIP),
                        std::move(pPort), std::move(manageIP), std::move(mPort),
                        4, i, std::move(metricPort)));  // 默认4线程启动服务
    }

    std::cout << "--------------------- Create single server done "
                 "----------------------"
              << std::endl;
    ...
    std::cout << "--------------------- create coodinator success "
                 "----------------------"
              << std::endl;
    ...
    nlohmann::json instancesInfo =
        SetPDRoleByList(predictIPList, predictPortList, managePortList,
                        interCommonPortLists, numPRole, numDRole);

    // 更新npu_mem_size
    for (auto i = 0; i < numPRole + numDRole; i++) {
        instancesInfo["instances"][i]["static_info"]["total_block_num"] =
            512 * (i + 1);  // 512 分配NPU内存大小
    ...
    auto lambdaFunc = [this, tlsItems, instancesInfo]() -> bool {
        std::string response;
        auto ret = SendInferRequest(this->manageIP, this->managePort, tlsItems,
                                    "/v1/instances/refresh",
                                    instancesInfo.dump(), response);
        std::cout << "------------ SetPDRole ret ----------------" << ret
                  << std::endl;
        if (ret == 0) {
            std::cout << "------------ SetPDRole Ready ----------------"
                      << std::endl;
            return true;
        } else {
            return false;
    ...
    std::cout << "--------------------- coodinator ready ----------------------"
              << std::endl;

    // 发送指标查询请求，默认超时时间60，最大重复请求次数3
    auto ret = GetMetricsRequest(manageIP, externalPort, tlsItems, "/metrics",
                                 response, false);
...
TEST_F(TestMetricsPDSeparate, TestPDSeparateMetricsTC02) {
    ...
    for (auto i = 0; i < numMindIEServer; i++) {
        ...
        threads.push_back(
            std::thread(CreateMetricsSSLServer, std::move(predictIP),
                        std::move(pPort), std::move(manageIP),
                        std::move(metricPort), 4, i));  // 默认4线程启动服务
    }
    ...
    tlsItems.tlsKey =
        GetAbsolutePath(certDir, "user_to_coordinator_manage/cert.key.pem");
    tlsItems.tlsPasswd =
        GetAbsolutePath(certDir, "user_to_coordinator_manage/cert_passwd.txt");

    auto lambdaFunc = [this, tlsItems, instancesInfo]() -> bool {
        std::string response;
        auto ret = SendSSLInferRequest(this->manageIP, this->managePort,
                                       tlsItems, "/v1/instances/refresh",
                                       instancesInfo.dump(), response);
        std::cout << "------------ SetPDRole ret ----------------" << ret
                  << std::endl;
        if (ret == 0) {
