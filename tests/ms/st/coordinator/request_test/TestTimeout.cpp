...
class TimeoutMock : public MindIEServerMock {
   public:
    explicit TimeoutMock(const std::string &keyInit)
        : MindIEServerMock(keyInit) {}
    ~TimeoutMock() = default;
    void SetPTimeout(size_t timeout) { pTimeout = timeout; }
    void SetDTimeout(size_t timeout) { dTimeout = timeout; }
    void SetPDRespond(std::shared_ptr<ServerConnection> connection,
                      bool isStream, std::string question,
        ...
        if (dInstance != nullptr) {
            ...
            MyThreadPool::GetInstance().submit(
                &MindIEServerMock::DMessageHandler, dInstance.get(), reqId,
                question, isStream);
        }
...
测试步骤:
    1.
将ms_coordinator.json中的配置项first_token_timeout设置成2，infer_timeout设置成5
...
TEST_F(TestTimeout, TestFirstTokenTimeoutTC01) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        threads.emplace_back(std::thread([&] {
            ...
            ClusterMock::GetCluster().AddInstance(predictIPList[i], pPort,
                                                  timeoutMock);
            // 注册请求到达时的回调函数
            handler1.RegisterFun(
                boost::beast::http::verb::post, "/v1/chat/completions",
                std::bind(&TimeoutMock::OpenAIPReqHandler, timeoutMock.get(),
                          std::placeholders::_1));
            handler1.RegisterFun(
                boost::beast::http::verb::get, "/dresult",
                std::bind(&TimeoutMock::DReqHandler, timeoutMock.get(),
                          std::placeholders::_1));
            ...
            handler2.RegisterFun(
                boost::beast::http::verb::post, "/v1/role/prefill",
                std::bind(&TimeoutMock::RolePHandler, timeoutMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::post, "/v1/role/decode",
                std::bind(&TimeoutMock::RoleDHandler, timeoutMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::get, "/v1/config",
                std::bind(&TimeoutMock::ConfigHandler, timeoutMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::get, "/v1/status",
                std::bind(&TimeoutMock::StatusHandler, timeoutMock.get(),
                          std::placeholders::_1));
...
TEST_F(TestTimeout, TestInferTimeoutTC01) {
    ...
    auto code = SendInferRequest(predictIP, predictPort, tlsItems,
                                 "/v1/chat/completions", streamBody, response);
    EXPECT_EQ(response,
              "{\"index\":0,\"value\":\"M\"}Request inference timeout\r\n");
    EXPECT_EQ(code, 0);
