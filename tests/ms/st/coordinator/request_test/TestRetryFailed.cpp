...
class RetryFailedMock : public MindIEServerMock {
   public:
    explicit RetryFailedMock(const std::string &keyInit)
    ...
    void SetPDRespond(std::shared_ptr<ServerConnection> connection,
        ...
        if (dInstance != nullptr) {
            ...
            if (req["is-recompute"] !=
                ...
                return ((RetryFailedMock *)dInstance.get())
                    ->DSendRetry(reqId, req);
            }
        }
...
TEST_F(TestRetryFailed, TestRetryFailedTC01) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        threads.emplace_back(std::thread([predictIP, pPort, manageIP, mPort] {
            ...
            handler1.RegisterFun(
                ...
                std::bind(&RetryFailedMock::OpenAIPReqHandler, retryMock.get(),
                          std::placeholders::_1));
            handler1.RegisterFun(
                boost::beast::http::verb::get, "/dresult",
                std::bind(&RetryFailedMock::DReqHandler, retryMock.get(),
                          std::placeholders::_1));
            ...
            handler2.RegisterFun(
                ...
                std::bind(&RetryFailedMock::RolePHandler, retryMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::post, "/v1/role/decode",
                std::bind(&RetryFailedMock::RoleDHandler, retryMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::get, "/v1/config",
                std::bind(&RetryFailedMock::ConfigHandler, retryMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::get, "/v1/status",
                std::bind(&RetryFailedMock::StatusHandler, retryMock.get(),
                          std::placeholders::_1));
    ...
    EXPECT_EQ(response,
              "{\"index\":0,\"value\":\"M\"}Request retry failed\r\n");
    EXPECT_EQ(code, 0);
