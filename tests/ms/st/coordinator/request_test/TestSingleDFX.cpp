...
class SingleResErrorMock : public MindIEServerMock {
   public:
    explicit SingleResErrorMock(const std::string &keyInit)
        : MindIEServerMock(keyInit) {}
...
class SingleTimeoutMock : public MindIEServerMock {
   public:
    explicit SingleTimeoutMock(const std::string &keyInit)
        : MindIEServerMock(keyInit) {}
    ~SingleTimeoutMock() = default;
    void SetFirstTimeout(size_t timeout) { this->firstTimeout = timeout; }
    void SetLastTimeout(size_t timeout) { this->lastTimeout = timeout; }
    void SingleTimeoutHandler(std::shared_ptr<ServerConnection> connection) {
        ...
        if (stream) {
            for (size_t i = 0; i < question.size(); ++i) {
                ...
                if (i == (question.size() - 1)) {
                    if (lastTimeout > 0) {
                        std::this_thread::sleep_for(
                            std::chrono::seconds(lastTimeout));
                    }
                    res.body += "reqId:" + reqId + '\0' +
                                "lastData:" + data.dump() + '\0';
                    res.isFinish = true;
                } else {
                    res.body =
                ...
                connection->SendRes(res);
                std::cout << "Stream res: " << reqId << ", data: " << res.body
                          << std::endl;
            }
        } else {
            if (firstTimeout > 0) {
...
class SingleRetryMock : public MindIEServerMock {
   public:
    explicit SingleRetryMock(const std::string &keyInit)
        : MindIEServerMock(keyInit) {}
...
std::string CreateOpenAIRequest(std::vector<std::string> contents,
                                bool stream) {
    // OpenAI request
...
std::unique_ptr<HttpServer> CreateSingleErrorServer(const std::string &ip1,
    ...
    handler1.RegisterFun(boost::beast::http::verb::post, "/v1/chat/completions",
                         std::bind(&SingleResErrorMock::SingleMessageHandler,
...
std::unique_ptr<HttpServer> CreateSingleTimeoutServer(
    const std::string &ip1, const std::string &port1, const std::string &ip2,
    const std::string &port2, int firstTimeout, int lastTimeout) {
    HttpServer httpServer1;
    httpServer1.Init(8);
    // 设置回调函数
    ServerHandler handler1;
    ...
    handler1.RegisterFun(boost::beast::http::verb::post, "/v1/chat/completions",
                         std::bind(&SingleTimeoutMock::SingleTimeoutHandler,
                                   openAIMock1.get(), std::placeholders::_1));
...
std::unique_ptr<HttpServer> CreateSingleRetryServer(const std::string &ip1,
                                                    const std::string &port1,
                                                    const std::string &ip2,
                                                    const std::string &port2) {
    HttpServer httpServer1;
    ...
    handler1.RegisterFun(boost::beast::http::verb::post, "/v1/chat/completions",
                         std::bind(&SingleRetryMock::SingleRetryHandler,
                                   openAIMock1.get(), std::placeholders::_1));
...
TEST_F(TestSingleDFX, TestSingleTimeoutTC01) {
    ...
    for (size_t i = 0; i < numMindIEServer; i++) {
        ...
        threads.push_back(std::thread(
            CreateSingleTimeoutServer, std::move(predictIP), std::move(pPort),
            std::move(manageIP), std::move(mPort), 6,
            11));  // 默认设置firstTimeout为6, lastTimeout 11线程启动服务
    }
...
TEST_F(TestSingleDFX, SingleInstancesAdd) {
    ...
    manager.SetList({"log_info", "run_log_path"},
                    "./runlog_SingleInstancesAdd.txt");
    manager.SetList({"log_info", "operation_log_path"},
                    "./operlog_SingleInstancesAdd.txt");
    manager.Save();
    ...
    for (size_t i = 0; i < numMindIEServer; i++) {
        ...
        threads.push_back(std::thread(
            CreateSingleServer, std::move(predictIP), std::move(pPort),
            std::move(manageIP), std::move(mPort), 4, i, ""));  // 默认4线程
    }
    ...
    EXPECT_EQ(ProcessStreamRespond(response),
              "My name is Olivier and IMy name is Olivier and I");
    EXPECT_EQ(code, 0);
...
TEST_F(TestSingleDFX, SingleInstancesUpdate) {
    ...
    manager.SetList({"log_info", "run_log_path"},
                    "./runlog_SingleInstancesUpdate.txt");
    manager.SetList({"log_info", "operation_log_path"},
                    "./operlog_SingleInstancesUpdate.txt");
    manager.Save();
...
TEST_F(TestSingleDFX, SingleInstancesRemove) {
    ...
    manager.SetList({"log_info", "to_file"}, true);
    manager.SetList({"log_info", "run_log_path"},
                    "./runlog_SingleInstancesRemove.txt");
    manager.SetList({"log_info", "operation_log_path"},
                    "./operlog_SingleInstancesRemove.txt");
    manager.Save();
