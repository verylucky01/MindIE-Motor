...
std::string CreateSelfDevelopRequest(std::string textInput, bool stream) {
    ...
    request["parameters"] = {
        ...
        {"max_new_tritons", 20},      {"do_sample", true}, {"seed", nullptr},
        {"repetition_penalty", 1.03}, {"details", true},   {"typical_p", 0.5},
        {"watermark", false}};
...
TEST_F(TestSelfDevelop, TestSelfDevelopTC01) {
    ...
    auto nonStreamBody =
        CreateSelfDevelopRequest("My name is Olivier and I", false);
    auto streamBody =
        CreateSelfDevelopRequest("My name is Olivier and I", true);
    ...
    auto code = SendInferRequest(predictIP, predictPort, tlsItems, "/infer",
                                 nonStreamBody, response);
    EXPECT_EQ(response,
              "My name is Olivier and I");  // mock实现为将所有的msg返回
    EXPECT_EQ(code, 0);

    code = SendInferRequest(predictIP, predictPort, tlsItems, "/infer",
                            streamBody, response);
