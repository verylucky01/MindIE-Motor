...
TEST_F(TestSSLRequest, TestSSLRequestTC01) {
    ...
    for (size_t i = 0; i < numMindIEServer; i++) {
        ...
        threads.push_back(std::thread(CreateSSLServer, std::move(predictIP),
                                      std::move(pPort), std::move(manageIP),
                                      std::move(mPort), 4, ""));  // 默认4线程
    }
    ...
    auto code =
        SendSSLInferRequest(predictIP, predictPort, tlsItems,
                            "/v1/chat/completions", nonStreamBody, response);
    EXPECT_EQ(response, "My name is Olivier and IOK");
    EXPECT_EQ(code, 0);

    code = SendSSLInferRequest(predictIP, predictPort, tlsItems,
