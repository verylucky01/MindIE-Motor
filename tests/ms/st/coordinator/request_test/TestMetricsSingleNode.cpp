...
class TestMetricsSingleNode : public testing::Test {
   protected:
    void SetUp() {
        ...
        msMetricsBin = GetAbsolutePath(
            parentPath, "tests/ms/common/.mindie_ms/ms_metrics.bin");
        ...
        std::string aggregate4SingleNodeOutput =
            GetAbsolutePath(parentPath,
                            "tests/ms/common/.mindie_ms/"
                            "ms_metricsAggregate4SingleNodeOutput.bin");
        ...
        manager.SetList({"prometheus_metrics_config", "reuse_time"},
                        10);  // 设置重用时间
...
TEST_F(TestMetricsSingleNode, TestSingleNodeMetricsTC01) {
    ...
    for (size_t i = 0; i < numServer; i++) {
        ...
        threads.push_back(
            std::thread(CreateSingleServer, std::move(predictIP),
    ...
    for (auto i = 0; i < numServer; i++) {
        ...
        static_info["total_block_num"] =
            512 * (i + 1);  // 不同Node分配不同的NPU MEM SIZE
    ...
    auto lambdaFunc = [this, tlsItems, instancesInfo]() -> bool {
        ...
        std::cout << "------------ SetSingleRole ret ----------------" << ret
                  << std::endl;
        if (ret == 0) {
            std::cout << "------------ SetSingleRole Ready ----------------"
                      << std::endl;
            return true;
        } else {
            return false;
...
class SortTimeReUseMock : public MindIEServerMock {
   public:
    explicit SortTimeReUseMock(const std::string &keyInit)
        : MindIEServerMock(keyInit) {}
    ~SortTimeReUseMock() = default;
    void MetricsHandler(std::shared_ptr<ServerConnection> connection) {
        std::cout << "--------------------- the " << requestCount
                  << " times request ----------------------" << std::endl;
        ServerRes res;
        res.body = singleNodeMetrics[requestCount %
                                     4];  // 4 根据请求顺序分配不同的指标数值
...
TEST_F(TestMetricsSingleNode, TestSingleNodeMetricsSortTimeReUseTC01) {
    std::vector<std::string> predictIPList = {"127.0.0.1", "127.0.0.1",
                                              "127.0.0.1", "127.0.0.1"};
    ...
    for (size_t i = 0; i < numServer; ++i) {
        ...
        threads.emplace_back(std::thread([predictIP, pPort, manageIP, mPort,
            ...
            // 注册请求到达时的回调函数
            handler1.RegisterFun(
                boost::beast::http::verb::post, "/v1/chat/completions",
                std::bind(&SortTimeReUseMock::OpenAISingleReqHandler,
            ...
            handler2.RegisterFun(boost::beast::http::verb::post,
                                 "/v1/role/prefill",
                                 std::bind(&SortTimeReUseMock::RolePHandler,
                                           mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::post,
                                 "/v1/role/decode",
                                 std::bind(&SortTimeReUseMock::RoleDHandler,
                                           mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::get, "/v1/config",
                                 std::bind(&SortTimeReUseMock::ConfigHandler,
                                           mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::get, "/v1/status",
                                 std::bind(&SortTimeReUseMock::StatusHandler,
                                           mock.get(), std::placeholders::_1));
            ...
            handler3.RegisterFun(boost::beast::http::verb::get, "/metrics",
                                 std::bind(&SortTimeReUseMock::MetricsHandler,
                                           mock.get(), std::placeholders::_1));
    ...
    // 发送第二次指标查询请求
    ret = GetMetricsRequest(manageIP, externalPort, tlsItems, "/metrics",
                            response2, false);
    EXPECT_EQ(ret, 200);  // 200 表示 HTTP 请求成功
