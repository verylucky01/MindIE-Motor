...
TEST_F(TestTokenizerAndProbe, TestTokenizerAndProbeTC01) {
    ...
    code = SendInferRequest(predictIP, predictPort, tlsItems, "/v1/tokenizer",
                            TokenizerBodyShort, response);
    EXPECT_EQ(
        response,
        "{\"token\":[\"M\"],\"token_number\":1}");  // mock实现为将所有的msg返回
    EXPECT_EQ(code, 0);

    // /v2/health/ready
    code = GetInferRequest(manageIP, statusPort, tlsItems, "/v2/health/ready",
                           response);
    EXPECT_EQ(code, 200);

    // /v1/coordinator_info
    code = GetInferRequest(manageIP, managePort, tlsItems,
                           "/v1/coordinator_info", response);
    EXPECT_EQ(code, 200);

    // /v1/instances/offline
    code = GetInferRequest(manageIP, managePort, tlsItems,
                           "/v1/instances/offline", response);
    EXPECT_EQ(code, 400);
    ...
    // /v1/startup
    code = GetInferRequest(manageIP, statusPort, tlsItems, "/v1/startup",
                           response);
    EXPECT_EQ(code, 200);

    // /v1/health
    code =
        GetInferRequest(manageIP, statusPort, tlsItems, "/v1/health", response);
    EXPECT_EQ(code, 200);

    // /v1/readiness
    code = GetInferRequest(manageIP, statusPort, tlsItems, "/v1/readiness",
                           response);
    EXPECT_EQ(code, 200);

    // /v2/health/live
    code = GetInferRequest(manageIP, statusPort, tlsItems, "/v2/health/live",
                           response);
    EXPECT_EQ(code, 200);

    // /v2/models/llama_65b/ready
    code = GetInferRequest(manageIP, statusPort, tlsItems,
                           "/v2/models/llama_65b/ready", response);
    EXPECT_EQ(code, 200);

    // /v2/models/gpt-3.5-turbo/ready, 模型不存在
    code = GetInferRequest(manageIP, statusPort, tlsItems,
                           "/v2/models/gpt-3.5-turbo/ready", response);
    EXPECT_EQ(code, 503);
    ...
    Stub stub3;
    stub3.set(ADDR(RequestListener, LinkWithDNodeInMaxRetry),
