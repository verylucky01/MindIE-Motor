...
class TestPResInvalid : public testing::Test {
   protected:
    void SetUp() {
        CopyDefaultConfig();
...
class PResInvalidMock : public MindIEServerMock {
   public:
    explicit PResInvalidMock(const std::string &keyInit)
        : MindIEServerMock(keyInit) {}
    ~PResInvalidMock() = default;
    void SetPDRespond(std::shared_ptr<ServerConnection> connection,
        ...
        ServerRes res;
        res.body = "PResInvalidMock return error";
...
*/
TEST_F(TestPResInvalid, TestPResInvalidTC01) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        threads.emplace_back(std::thread([predictIP, pPort, manageIP, mPort]() {
            ...
            handler2.RegisterFun(boost::beast::http::verb::post,
                                 "/v1/role/prefill",
                                 std::bind(&PResInvalidMock::RolePHandler,
                                           mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::post,
                                 "/v1/role/decode",
                                 std::bind(&PResInvalidMock::RoleDHandler,
                                           mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::get, "/v1/config",
                                 std::bind(&PResInvalidMock::ConfigHandler,
                                           mock.get(), std::placeholders::_1));
            handler2.RegisterFun(boost::beast::http::verb::get, "/v1/status",
                                 std::bind(&PResInvalidMock::StatusHandler,
                                           mock.get(), std::placeholders::_1));
    ...
    EXPECT_EQ(response, "P instance error\r\n");
    EXPECT_EQ(code, 0);
