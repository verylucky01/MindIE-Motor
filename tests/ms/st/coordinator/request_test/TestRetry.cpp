...
class RetryMock : public MindIEServerMock {
   public:
    explicit RetryMock(const std::string &keyInit)
...
TEST_F(TestRetry, TestRetryTC01) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        threads.emplace_back(std::thread([predictIP, pPort, manageIP, mPort] {
            ...
            handler1.RegisterFun(
                ...
                std::bind(&RetryMock::OpenAIPReqHandler, retryMock.get(),
                          std::placeholders::_1));
            handler1.RegisterFun(
                boost::beast::http::verb::get, "/dresult",
                std::bind(&RetryMock::DReqHandler, retryMock.get(),
                          std::placeholders::_1));
            ...
            handler2.RegisterFun(
                ...
                std::bind(&RetryMock::RolePHandler, retryMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::post, "/v1/role/decode",
                std::bind(&RetryMock::RoleDHandler, retryMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::get, "/v1/config",
                std::bind(&RetryMock::ConfigHandler, retryMock.get(),
                          std::placeholders::_1));
            handler2.RegisterFun(
                boost::beast::http::verb::get, "/v1/status",
                std::bind(&RetryMock::StatusHandler, retryMock.get(),
                          std::placeholders::_1));
    ...
    auto lamda = [&]() { __main_coordinator__(1, argsCoordinator); };
    ...
    EXPECT_EQ(ProcessStreamRespond(response),
              "MMy name is Olivier and I");  // mock实现为将所有的msg拼接
    EXPECT_EQ(code, 0);
