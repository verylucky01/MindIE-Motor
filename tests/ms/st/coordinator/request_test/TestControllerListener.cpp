...
class TestControllerListener : public ::testing::Test {
   public:
    ...
    static void SetUpTestCase() {
        CopyFile(GetMSCoordinatorConfigJsonPath(),
                 GetMSCoordinatorTestJsonPath());
        auto coordinatorJson = GetMSCoordinatorTestJsonPath();

        std::cout << "coordinatorJson:" << coordinatorJson << std::endl;
        setenv("MINDIE_MS_COORDINATOR_CONFIG_FILE_PATH",
               coordinatorJson.c_str(), 1);
        setenv("HSECEASY_PATH", GetHSECEASYPATH().c_str(), 1);
        std::cout << "MINDIE_MS_COORDINATOR_CONFIG_FILE_PATH="
                  << coordinatorJson << std::endl;
        ...
        for (size_t i = 0; i < 4; ++i) {
            ...
            auto thread = std::make_unique<std::thread>(
                CreateMindIEServer, std::move(predictIP), std::move(pPort),
                std::move(manageIP), std::move(mPort), 4, i,
                "");  // 默认4线程启动服务
        ...
        auto threadObj = std::make_unique<std::thread>(__main_coordinator__, 1,
                                                       argsCoordinator);
        g_testControllerListenerthreadsList.emplace_back(std::move(threadObj));
        std::string response;
        auto pdInstance =
            SetPDRole(g_predictIPList, g_predictPortList, g_managePortList,
                      g_interCommonPortLists, 2, 2, g_manageIP, g_managePort);
        WaitCoordinatorReady(g_manageIP, g_statusPort);
    }

    void SetUp() override {
        initInfo =
            SetPDRoleByList(g_predictIPList, g_predictPortList,
                            g_managePortList, g_interCommonPortLists, 2, 2)
                .dump();  // 2和2分别是P和D的比例
        logDirPath = GetMSCoordinatorLogPath();
        runLogPath = GetMSCoordinatorRunLogPath();
        operationLogPath = GetMSCoordinatorOperationLogPath();
    ...
    static void TearDownTestCase() {
        for (size_t i = 0; i < g_testControllerListenerthreadsList.size();
             ++i) {
    ...
    std::string ModifyJsonForTest(std::string Info,
                                  std::function<void(json &)> modifyFunc) {
        json j = json::parse(Info);
        modifyFunc(j);
        return j.dump();
    }

    bool SearchLogFileForTarget(const std::string &logFileName,
...
std::string DeleteServer(std::string serverLists, uint8_t serverID) {
    ...
    for (auto it = j["instances"].begin(); it != j["instances"].end();) {
        if (it->at("id") == serverID) {
            ...
            // 删除ids
            j["ids"].erase(
                std::remove(j["ids"].begin(), j["ids"].end(), serverID),
                j["ids"].end());
        } else {
            ++it;
        }
...
int32_t SendInstanceRefreshInfo(std::string &g_manageIP,
                                std::string &g_managePort,
                                std::string instancesInfo) {
    TlsItems tlsItems;
    std::string response;
    // 发送更新后的集群状态信息
    auto ret =
        SendInferRequest(g_manageIP, g_managePort, tlsItems,
                         "/v1/instances/refresh", instancesInfo, response);
    return ret;
}
...
测试步骤:
    ...
    5. 删除一个Device (D)
后，发送包含2P1D的集群状态信息，coordinator不可用，预期结果1。
    6. 删除两个Device (D)
后，发送包含2P0D的集群状态信息，coordinator不可用，预期结果1。 预期结果:
    1. 发送集群状态信息成功，返回值为0。
*/
TEST_F(TestControllerListener, InstancesRefreshHandlerTest) {
    ...
    auto ret = GetInferRequest(g_manageIP, g_statusPort, tlsItems,
                               "/v2/health/ready", response);
    EXPECT_EQ(ret, 200);
    ...
    ret = SendInstanceRefreshInfo(g_manageIP, g_managePort, newInfo);
    EXPECT_EQ(ret, 0);
    ret = GetInferRequest(g_manageIP, g_statusPort, tlsItems,
                          "/v2/health/ready", response);
    EXPECT_EQ(ret, 503);
...
TEST_F(TestControllerListener, RefreshRequestErrorTC01) {
    ...
    std::string modifiedInfo =
        ModifyJsonForTest(initInfo, [](json &j) { j.erase("ids"); });
...
TEST_F(TestControllerListener, RefreshRequestErrorTC02) {
    ...
    // 修改ids字段为单个整数
    std::string modifiedInfo =
        ModifyJsonForTest(initInfo, [](json &j) { j["ids"] = j["ids"][0]; });
...
TEST_F(TestControllerListener, RefreshRequestErrorTC03) {
    ...
    // 删除instances字段
    std::string modifiedInfo =
        ModifyJsonForTest(initInfo, [](json &j) { j.erase("instances"); });
...
TEST_F(TestControllerListener, RefreshRequestErrorTC04) {
    ...
    // 修改instances为单个对象
    std::string modifiedInfo = ModifyJsonForTest(
        initInfo, [](json &j) { j["instances"][0].erase("id"); });
...
TEST_F(TestControllerListener, RefreshRequestErrorTC05) {
    ...
    std::string modifiedInfo = ModifyJsonForTest(initInfo, [](json &j) {
        j["ids"].insert(j["ids"].begin(), 123456789);
    });
...
json CreateNewInstance() {
    return json{
        {"dynamic_info", {{"avail_block_num", 1024}, {"avail_slots_num", 200}}},
        {"id", 5},
        {"ip", "127.0.0.1"},
...
TEST_F(TestControllerListener, RefreshRequestErrorTC07) {
    ...
    std::string target =
        "Remove D instance IP: 127.0.0.1:" + g_predictPortList[3];
...
TEST_F(TestControllerListener, RefreshRequestErrorTC08) {
    ...
    // 删除instances里id为3的实例
    std::string modifiedInfo = ModifyJsonForTest(
        initInfo, [](json &j) { j["instances"].erase(j["instances"].end()); });
...
TEST_F(TestControllerListener, RefreshRequestErrorTC09) {
    ...
    std::string modifiedInfo =
        ModifyJsonForTest(initInfo, [](json &j) { j["ids"] = {-1, 1, 2, 3}; });
    ...
    std::string target =
        "Remove P instance IP: 127.0.0.1:" + g_predictPortList[0];
    bool found = SearchLogFileForTarget("./operlog.txt", target);
    EXPECT_TRUE(found);
}
...
TEST_F(TestControllerListener, RefreshRequestErrorTC10) {
    ...
    std::string modifiedInfo = ModifyJsonForTest(
        initInfo, [](json &j) { j["instances"][3]["id"] = -1; });
...
TEST_F(TestControllerListener, RefreshRequestErrorTC14) {
    std::string modifiedInfo = ModifyJsonForTest(
        initInfo, [](json &j) { j["instances"] = {"123", "456", "789"}; });
...
/*
测试描述:
PD分离场景，分别同步信息中的P实例和D实例，在coordinator不存在，扩容场景
测试步骤:
    1. 初始化 TLS 项和响应字符串。
...
TEST_F(TestControllerListener, InstancesRefreshHandlerExpansion) {
    ...
    newInfo = DeleteServer(newInfo, 3);
    auto ret =
        SendInstanceRefreshInfo(g_manageIP, g_managePort,
                                newInfo);  // 更新集群状态信息，并发送到管理节点
    EXPECT_EQ(ret, 0);
    ret =
        GetInferRequest(g_manageIP, g_statusPort, tlsItems, "/v2/health/ready",
                        response);  // 发送健康检查请求，确认服务准备就绪
    EXPECT_EQ(ret, 200);

    // 创建流式和非流式请求体
    auto nonStreamBody =
        CreateOpenAIRequest({"My name is Olivier and I", "OK"}, false);
    auto streamBody = CreateOpenAIRequest(
        {"My name is Olivier and I", "My name is Olivier and I"}, true);
    ...
    // 发送推理请求
    auto code =
        SendInferRequest(g_predictIP, g_predictPort, tlsItems,
                         "/v1/chat/completions", nonStreamBody, response);
    EXPECT_EQ(response,
              "My name is Olivier and IOK");  // mock实现为将所有的msg拼接
    EXPECT_EQ(code, 0);
    code = SendInferRequest(g_predictIP, g_predictPort, tlsItems,
                            "/v1/chat/completions", streamBody, response);
    EXPECT_EQ(
        ProcessStreamRespond(response),
        "My name is Olivier and IMy name is Olivier and I");  // mock实现为将所有的msg拼接
    EXPECT_EQ(code, 0);
    code = SendInferRequest(g_predictIP, g_predictPort, tlsItems,
                            "/v1/chat/completions", singlestreamBody, response);
    EXPECT_EQ(ProcessStreamRespond(response),
              "M");  // mock实现为将所有的msg拼接
    EXPECT_EQ(code, 0);
    code =
        SendInferRequest(g_predictIP, g_predictPort, tlsItems,
                         "/v1/chat/completions", singlenonstreamBody, response);
    ...
    target = "Add instance D IP 127.0.0.1:" +
             g_predictPortList[3];  // 3: interval port
...
TEST_F(TestControllerListener, InstancesRefreshHandlerContraction) {
    ...
    target = "Remove D instance IP: 127.0.0.1:" +
             g_predictPortList[3];  // 3: internal port
    found = SearchLogFileForTarget("./operlog.txt", target);
    EXPECT_TRUE(found);
}
...
TEST_F(TestControllerListener, InstancesRefreshHandlerInvalid) {
    ...
    // 修改实例3信息，实例3无效
    std::string modifiedInfo = ModifyJsonForTest(
        newInfo, [](json &j) { j["instances"][2]["ip"] = "invalid_ip"; });
    ret = SendInstanceRefreshInfo(g_manageIP, g_managePort, modifiedInfo);
...
int32_t SendCoordinatorInfo(std::string &g_manageIP, std::string &g_managePort,
                            std::string instancesInfo) {
    TlsItems tlsItems;
    std::string response;
    auto ret = GetInferRequest(g_manageIP, g_managePort, tlsItems,
                               "/v1/coordinator_info", response);
    return ret;
}
...
测试步骤:
    ...
    5. 删除一个Device (D)
后，发送包含2P1D的协调器信息，coordinator不可用，预期结果1。
    6. 删除两个Device (D)
后，发送包含2P0D的协调器信息，coordinator不可用，预期结果1。 预期结果:
...
int32_t SendInstancesOfflineInfo(std::string &g_manageIP,
                                 std::string &g_managePort,
                                 std::string instancesInfo) {
    TlsItems tlsItems;
    std::string response;
    auto ret =
        SendInferRequest(g_manageIP, g_managePort, tlsItems,
                         "/v1/instances/offline", instancesInfo, response);
    return ret;
}
...
测试步骤:
    ...
    5. 删除一个Device (D)
后，发送包含2P1D的实例离线信息，coordinator不可用，预期结果1。
    6. 删除两个Device (D)
后，发送包含2P0D的实例离线信息，coordinator不可用，预期结果1。 预期结果:
    1. 发送实例离线信息成功，返回值为0。
*/
...
int32_t SendInstancesOnlineInfo(std::string &g_manageIP,
                                std::string &g_managePort,
                                std::string instancesInfo) {
    TlsItems tlsItems;
    std::string response;
    auto ret =
        SendInferRequest(g_manageIP, g_managePort, tlsItems,
                         "/v1/instances/online", instancesInfo, response);
    return ret;
}
...
测试步骤:
    ...
    5. 删除一个Device (D)
后，发送包含2P1D的实例在线信息，coordinator不可用，预期结果1。
    6. 删除两个Device (D)
后，发送包含2P0D的实例在线信息，coordinator不可用，预期结果1。 预期结果:
    1. 发送实例在线信息成功，返回值为0。
*/
...
int32_t SendInstancesTasksInfo(std::string &g_manageIP,
                               std::string &g_managePort,
                               std::string instancesInfo) {
    TlsItems tlsItems;
    std::string response;
    auto ret = GetInferRequest(g_manageIP, g_managePort, tlsItems,
                               "/v1/instances/tasks*", response);
    return ret;
}
...
int32_t SendStartUpProbeInfo(std::string &g_manageIP, std::string &g_managePort,
                             std::string instancesInfo) {
    TlsItems tlsItems;
    std::string response;
    auto ret = GetInferRequest(g_manageIP, g_managePort, tlsItems,
...
int32_t HealthProbeInfo(std::string &g_manageIP, std::string &g_managePort,
                        std::string instancesInfo) {
    TlsItems tlsItems;
    std::string response;
    auto ret = GetInferRequest(g_manageIP, g_managePort, tlsItems, "/v1/health",
                               response);
    return ret;
}
...
int32_t SendReadinessProbeInfo(std::string &g_manageIP,
                               std::string &g_managePort,
                               std::string instancesInfo) {
    TlsItems tlsItems;
    std::string response;
...
int32_t SendTritonModelsReadyInfo(std::string &g_manageIP,
                                  std::string &g_managePort,
                                  std::string instancesInfo) {
    TlsItems tlsItems;
    std::string response;
    auto ret = GetInferRequest(g_manageIP, g_managePort, tlsItems,
                               "/v1/readiness", response);
    return ret;
}
...
int32_t SendInstancesQueryTasksInfo(std::string &g_manageIP,
                                    std::string &g_managePort,
                                    std::string instancesInfo) {
    TlsItems tlsItems;
    std::string response;
    auto ret =
        SendInferRequest(g_manageIP, g_managePort, tlsItems,
                         "/v1/instances/query_tasks", instancesInfo, response);
    return ret;
}
...
TEST_F(TestControllerListener, InstancesQueryTasksHandlerTest) {
...

    {
        ...
        // 发送查询任务状态信息
        auto ret =
            SendInstancesQueryTasksInfo(g_manageIP, g_managePort, initInfo);
        EXPECT_EQ(ret, 0);
    }
    // Pid不存在
    {
...
/*
测试描述:
向ControllerListener发送refresh请求更新集群信息，请求中inter_comm_port字段缺失
测试步骤:
    1. 初始化 TLS 项和响应字符串
    2. 发送合法的初始状态，预期结果1
...
预期结果:
    1. 发送集群状态信息成功，返回值为200
    2. 发送集群状态信息成功，但集群不可用，coordinator非健康，日志输出 "key
'inter_comm_port' not found"
*/
TEST_F(TestControllerListener, RefreshWithoutInterCommPort) {
    ...
    auto modFunc = [](json &j) {
        for (auto &ins : j["instances"]) {
            // 修改身份触发实例端口信息刷新
            auto newRole =
                ins["static_info"]["role"] == int('P') ? int('D') : int('P');
...
/*
测试描述:
向ControllerListener发送refresh请求更新集群信息，请求中inter_comm_port字段非法
测试步骤:
    1. 初始化 TLS 项和响应字符串
    2. 发送合法的初始状态，预期结果1
...
预期结果:
    ...
    2. 发送集群状态信息成功，inter_comm_port字段使用默认空值，
        集群任然可用，日志输出 inter_comm_port "will not be used for routing
inference requests"
*/
    ...
    auto modFunc = [](json &j) {
        for (auto &ins : j["instances"]) {
            ...
            auto newRole =
                ins["static_info"]["role"] == int('P') ? int('D') : int('P');
            ins["static_info"]["role"] = newRole;
            int32_t curPort = static_cast<int32_t>(
                std::stoi(ins["inter_comm_port"].get<std::string>()));
            curPort += 65535;
...
/*
测试描述:
向ControllerListener发送refresh请求更新集群信息，请求中inter_comm_port字段为空
测试步骤:
    1. 初始化 TLS 项和响应字符串
    2. 发送合法的初始状态，预期结果1
...
TEST_F(TestControllerListener, AbnormalStatusHandlerTestWithValidJson) {
    ...
    std::string body =
        R"({"is_master":true,"is_abnormal":false,"is_random_pick":true})";
    int32_t ret = SendInferRequest(g_manageIP, g_managePort, tlsItems,
                                   "/backup_info", body, response);
    EXPECT_EQ(ret, 0);
}
...
TEST_F(TestControllerListener, AbnormalStatusHandlerTestWithInValidJson) {
    ...
    std::string body =
        R"({"is_master":true,"is_abnormal":false,)";  // 非法 JSON
    int32_t ret = SendInferRequest(g_manageIP, g_managePort, tlsItems,
                                   "/backup_info", body, response);
    EXPECT_EQ(ret, 0);
}
...
TEST_F(TestControllerListener, RecvsUpdaterTest) {
    ...
    int32_t ret = GetInferRequest(g_manageIP, g_managePort, tlsItems,
                                  "/recvs_info", response);
    EXPECT_EQ(ret, 200);
}
