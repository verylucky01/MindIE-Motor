...
class TestInferReq : public testing::Test {
   protected:
    void SetUp() override {
        CopyDefaultConfig();
...
class TestInferReqPrompt : public TestInferReq,
                           public ::testing::WithParamInterface<PromptParam> {
    void SetUp() { TestInferReq::SetUp(); }
};
...
TEST_P(TestInferReqPrompt, TestInferReqTC01) {
    auto param = GetParam();
...
TEST_P(TestInferReqPrompt, TestInferReqTC02) {
    auto coordinatorJson = GetMSCoordinatorTestJsonPath();
    JsonFileManager manager(coordinatorJson);
    manager.Load();
...
INSTANTIATE_TEST_SUITE_P(TestInferReqPrompts, TestInferReqPrompt,
