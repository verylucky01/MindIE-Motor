    ...
    void SetUp() override {
        ...
        for (auto i = 0; i < numMindIEServer; i++) {
            ...
            auto thread = std::make_unique<std::thread>(
                CreateMindIEServer, std::move(serverPIp), std::move(pPort),
                std::move(serverMIp), std::move(mPort), 4, i,
                "");  // 默认4线程启动服务
...
/*
测试描述:
测试2P2D的正常场景下，开启全零监听后可以正常接受推理请求，返回预期的推理结果。
测试步骤:
    1. 初始化 TLS 项和响应字符串。
    2.
向127.0.0.1:Coordinator的管理面端口发送包含2P2D的集群状态信息，预期结果1。
...
TEST_F(TestAllZeroIpListening, ZeroIpListeningInferSuccess) {
    ...
    // 发送推理请求
    auto code =
        SendInferRequest(localHost, predictPort, tlsItems,
                         "/v1/chat/completions", nonStreamBody, response);
    EXPECT_EQ(response,
              "My name is Olivier and IOK");  // mock的server会返回prompt的拼接
    EXPECT_EQ(code, 0);
}
