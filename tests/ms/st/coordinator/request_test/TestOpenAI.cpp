...
class TestOpenAI : public testing::Test {
    ...
    std::vector<std::thread> StartServices() {
        ...
        for (size_t i = 0; i < numMindIEServer; i++) {
            ...
            std::cout << "ip = " << predictIPList[i]
                      << ", predict port = " << pPort
                      << ", manage port = " << mPort << std::endl;

            threads.push_back(
                std::thread(CreateMindIEServer, std::move(manageIP),
                            std::move(pPort), std::move(predictIP),
                            std::move(mPort), 4, i, ""));  // 默认4线程启动服务
        }
...
预期结果:
    ...
    2. 将响应结果合并为完整内容为"My name is Olivier and IMy name is Olivier and
I"，返回值code应为0（表示成功）
    3. 流式响应通过 ProcessStreamRespond 处理后，预期内容为 "M"
    4. 直接断言响应值 response 为 "M"
    5. 返回缺少关键字错误信息"Invalid request format: Missing both 'prompt' or
'messages'\r\n"
...
TEST_F(TestOpenAI, TestOpenAITC01) {
    ...
    auto exceptionBody =
        CreateOpenAIExceptionRequest({"My name is Olivier and I", "OK"}, false);
    ...
    code = SendInferRequest(predictIP, predictPort, tlsItems,
                            "/v1/chat/completions", exceptionBody, response);
    EXPECT_EQ(
        response,
...
预期结果:
    ...
    2. 将响应结果合并为完整内容为"My name is Olivier and
I."，返回值code应为0（表示成功）
    3. 流式响应通过 ProcessStreamRespond 处理后，预期内容为 "M"
    4. 直接断言响应值 response 为 "M"
    5. 返回缺少关键字错误信息"Invalid request format: Missing both 'prompt' or
...
TEST_F(TestOpenAI, TestOpenAICompletionsTC01) {
    ...
    auto nonStreamBody =
        CreateOpenAICompletionsRequest("My name is Olivier and I.", false);
    auto streamBody =
        CreateOpenAICompletionsRequest("My name is Olivier and I.", true);
    ...
    auto exceptionBody = CreateOpenAICompletionsExceptionRequest(
        "My name is Olivier and I.", false);
    ...
    auto code = SendInferRequest(predictIP, predictPort, tlsItems,
                                 "/v1/completions", nonStreamBody, response);
    EXPECT_EQ(response, "My name is Olivier and I.");
    EXPECT_EQ(code, 0);

    code = SendInferRequest(predictIP, predictPort, tlsItems, "/v1/completions",
                            streamBody, response);
