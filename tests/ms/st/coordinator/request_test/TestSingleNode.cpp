...
TEST_F(TestSingleNode, TestDefaultPrefixCacheTC02) {
    ...
    auto code = SendInferRequest(predictIP, predictPort, tlsItems,
                                 "/v1/tokenizer", TokenizerBody, response);
    EXPECT_EQ(
        response,
        "{\"token\":[\"My name is Olivier and I\"],\"token_number\":24}");  // mock实现为将所有的msg返回
    EXPECT_EQ(code, 0);
...
TEST_F(TestSingleNode, TestDefaultPrefixCacheTC03) {
    ...
    auto code = SendInferRequest("127.0.0.1", "1025", tlsItems,
    ...
    code = SendInferRequest(predictIP, predictPort, tlsItems,
                            "/v2/models/llama_65b/generate", body.dump(),
                            response);
    EXPECT_EQ(response,
              "My name is Olivier and I");  // mock实现为将所有的msg返回
    EXPECT_EQ(code, 0);
    ...
    code =
        SendInferRequest(predictIP, predictPort, tlsItems,
                         "/v2/models/llama_65b/infer", body.dump(), response);
...
TEST_F(TestSingleNode, TestDefaultRoundRobinTC01) {
    ...
    auto code = SendInferRequest(predictIP, predictPort, tlsItems,
                                 ...
                                 body.dump(), response);
    EXPECT_EQ(ProcessStreamRespond(response),
              "My name is Olivier and I");  // mock实现为将所有的msg拼接
    EXPECT_EQ(code, 0);
