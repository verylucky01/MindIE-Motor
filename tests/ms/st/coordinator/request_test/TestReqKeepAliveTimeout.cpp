...
class TestReqKeepAliveTimeout : public testing::Test {
   protected:
    void SetUp() {
        ...
        manager.SetList({"http_config", "keep_alive_seconds"},
                        5);  // 长链接的时间改短到5秒，防止用例执行太久
        manager.Save();
    }
...
static void ServerExceptionCloseHandlerStub(
...
TEST_F(TestReqKeepAliveTimeout, TestReqKeepAliveTimeoutTC01) {
    ...
    auto pdInstance = SetPDRole(predictIPList, predictPortList, managePortList,
                                interCommonPortLists, 2, 2, manageIP,
                                managePort);  // 2表示PD两种类型
    WaitCoordinatorReady(manageIP, managePort);
    auto reqBody = CreateOpenAIRequest(
    ...
    clientHandler.RegisterFun(
        ClientHandlerType::CHUNK_BODY_RES,
        [&](std::shared_ptr<ClientConnection> connection) {
            boost::beast::string_view body = connection->GetResChunkedBody();
            results += body;
        });
    Stub stub;
    stub.set(ADDR(RequestListener, ServerExceptionCloseHandler),
             &ServerExceptionCloseHandlerStub);
    uint32_t id;
    auto ret =
        httpClient.AddConnection(predictIP, predictPort, id, clientHandler);
    ASSERT_TRUE(ret);
    ...
    if (g_cv.wait_for(lock, std::chrono::seconds(10)) ==
        std::cv_status::no_timeout) {  // 最多10秒
        EXPECT_EQ(ProcessStreamRespond(results),
...
TEST_F(TestReqKeepAliveTimeout, TestReqKeepAliveTimeoutTC02) {
    ...
    auto reqBody = CreateOpenAIRequest(
        {"My name is Olivier and I", "My name is Olivier and I"}, false);
    std::string results = "";
    std::mutex mtx;
    HttpClientAsync httpClient;
    TlsItems tls;
