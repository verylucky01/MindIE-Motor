...
int64_t GetMemoryLimitStub() { return g_memoryLimit; }

int64_t GetMemoryUsageStub() { return g_memoryUsage; }
    ...
    std::vector<std::thread> StartServices() {
        ...
        for (size_t i = 0; i < numMindIEServer; i++) {
            ...
            threads.push_back(std::thread(
                CreateMindIEServer, std::move(manageIP), std::move(pPort),
                std::move(predictIP), std::move(mPort), 4, i, ""));
        }
        return threads;
    }
 ...
 * 通过模拟高内存压力，触发 PreRequestCheck -> ShouldIntercept -> LogIntercept
 * 调用链 测试步骤:
...
TEST_F(TestRequestListener, LogInterceptMemoryPressure) {
    ...
    Stub stubLimit;
    Stub stubUsage;
    stubLimit.set(ADDR(MINDIE::MS::MemoryUtil, GetMemoryLimit),
                  GetMemoryLimitStub);
    stubUsage.set(ADDR(MINDIE::MS::MemoryUtil, GetMemoryUsage),
                  GetMemoryUsageStub);
    ...
    // 预期: PreRequestCheck 返回 false，ShouldIntercept 被调用，LogIntercept
    // 被调用
    auto code =
