...
static void OpenAIStaticCacheAffinity(const std::string &predictIP,
    ...
    std::string inferString =
        CreateOpenAIRequest({"hello1", "hello2", "hello3"}, false);
...
TEST_F(StressTestSingleNode, TestSingleNodeOpenAIStressTC01) {
    ...
    StressTetsSingleNodeHelper(mindieServerNum, OpenAIStaticCacheAffinity,
                               MetricsRequest, numThreadInfer);
}

// OpenAI固定请求, 激活cache_affinity, 激活round_robin
static void OpenAIStaticRoundRobin(const std::string &predictIP,
                                   const std::string &predictPort) {
    std::string response;
    TlsItems tlsItems;
    std::string inferUrl = "/v1/chat/completions";
...
TEST_F(StressTestSingleNode, TestSingleNodeOpenAIStressTC02) {
    uint32_t mindieServerNum = 8;
    uint32_t numThreadInfer = 5000;
    StressTetsSingleNodeHelper(mindieServerNum, OpenAIStaticRoundRobin,
                               MetricsRequest, numThreadInfer);
}
