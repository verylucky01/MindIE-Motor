...
using MetricsReqTestFun = std::function<void(
    const std::string &, const std::string &, const std::string &)>;

// PD分离压测
void StressTetsPDHelper(uint32_t numMindIEServer, ReqTestFun reqTestFun,
                        MetricsReqTestFun metricsFun,
                        uint32_t numThreadInfer = 10, uint32_t pRate = 0,
                        uint32_t dRate = 0) {
    std::cout << "---------------------StressTetsPDHelper Test start "
                 "----------------------"
              << std::endl;
    ...
    // 设置启动环境变量
    setenv("MINDIE_MS_COORDINATOR_CONFIG_FILE_PATH", coordinatorJson.c_str(),
           1);
    ...
    std::cout << "MINDIE_MS_COORDINATOR_CONFIG_FILE_PATH=" << coordinatorJson
              << std::endl;
    std::cout << "MINDIE_MS_CONTROLLER_CONFIG_FILE_PATH=" << controllerJson
              << std::endl;
    ...
    // 修改启动配置文件
    {
        ...
        manager.SetList({"http_config", "predict_port"},
                        std::to_string(predictPort));
        manager.SetList({"http_config", "manage_ip"}, manageIP);
        manager.SetList({"http_config", "manage_port"},
                        std::to_string(managePort));
    ...
    std::string stressPDSeparateData = GetAbsolutePath(
        parentPath,
        "tests/ms/common/.mindie_ms/ms_metricsPDSeparateNodeData.bin");
    ...
    for (auto index = 0; index < numMindIEServer; index++) {
        std::cout << "ip = " << predictIPList[index]
                  << ", predict port = " << predictPort
                  << ", manage port = " << managePort
                  << ", metircs port = " << metricsPort << std::endl;
        threads.emplace_back(std::thread([&] {
            CreateMindIEServer(
                predictIPList[index], std::to_string(predictPort),
                predictIPList[index], std::to_string(managePort), 4, 0,
                std::to_string(metricsPort));  // 4 线程 0 编号
        }));
        sleep(5);
    }
    ...
    for (auto index = 0; index < numThreadInfer; index++) {
        inferThreads.emplace_back(std::thread([&] {
            ...
            metricsFun(manageIP, std::to_string(managePort),
                       metricsStressTestPDSeparate);
        }));
    }
    ...
    std::cout << "---------------------StressTetsPDHelper Test end "
                 "----------------------"
              << std::endl;
}

// PD分离TLS压测
void StressTetsPDTLSHelper(uint32_t numMindIEServer, ReqTestFun reqTestFun,
                           MetricsReqTestFun metricsFun,
                           uint32_t numThreadInfer = 10, uint32_t pRate = 0,
                           uint32_t dRate = 0) {
    std::cout << "---------------------StressTetsPDSSLHelper Test start "
                 "----------------------"
              << std::endl;
    ...
    // 修改启动配置文件
    {
        ...
        manager.SetList({"log_info", "run_log_path"},
                        "./logs/ms_coordinator_run_log.txt");
        manager.SetList({"log_info", "operation_log_path"},
                        "./logs/ms_coordinator_operation_log.txt");
        manager.SetList({"http_config", "server_thread_num"}, 4);
        manager.SetList({"http_config", "client_thread_num"}, 4);
        manager.SetList({"http_config", "predict_ip"}, predictIP);
        manager.SetList({"http_config", "predict_port"},
        ...
        manager.SetList(
            {"tls_config", "request_server_tls_items", "tls_cert"},
            GetAbsolutePath(certDir, "coordinator_to_user/cert.pem"));
        manager.SetList(
            {"tls_config", "request_server_tls_items", "tls_key"},
            GetAbsolutePath(certDir, "coordinator_to_user/cert.key.pem"));
        manager.SetList(
            {"tls_config", "request_server_tls_items", "tls_passwd"},
        ...
        manager.SetList(
            {"tls_config", "mindie_client_tls_items", "ca_cert"},
            GetAbsolutePath(certDir, "coordinator_to_mindie_server/ca.pem"));
        manager.SetList(
            {"tls_config", "mindie_client_tls_items", "tls_cert"},
            GetAbsolutePath(certDir, "coordinator_to_mindie_server/cert.pem"));
        manager.SetList(
            {"tls_config", "mindie_client_tls_items", "tls_key"},
            GetAbsolutePath(certDir,
                            "coordinator_to_mindie_server/cert.key.pem"));
        manager.SetList(
            {"tls_config", "mindie_client_tls_items", "tls_passwd"},
            GetAbsolutePath(certDir,
                            "coordinator_to_mindie_server/cert_passwd.txt"));
        manager.SetList({"tls_config", "mindie_client_tls_items", "tls_crl"},
                        "");

        manager.SetList({"tls_config", "controller_server_tls_enable"}, true);
        manager.SetList(
            {"tls_config", "controller_server_tls_items", "ca_cert"},
            GetAbsolutePath(certDir, "coordinator_to_controller/ca.pem"));
        manager.SetList(
            {"tls_config", "controller_server_tls_items", "tls_cert"},
            GetAbsolutePath(certDir, "coordinator_to_controller/cert.pem"));
        manager.SetList(
            {"tls_config", "controller_server_tls_items", "tls_key"},
            GetAbsolutePath(certDir, "coordinator_to_controller/cert.key.pem"));
        manager.SetList(
            {"tls_config", "controller_server_tls_items", "tls_passwd"},
            GetAbsolutePath(certDir,
                            "coordinator_to_controller/cert_passwd.txt"));
        manager.SetList(
            {"tls_config", "controller_server_tls_items", "tls_crl"}, "");

        manager.SetList({"tls_config", "mindie_mangment_tls_enable"}, true);
        manager.SetList(
            {"tls_config", "mindie_mangment_tls_items", "ca_cert"},
            GetAbsolutePath(certDir,
                            "coordinator_to_mindie_server_manage/ca.pem"));
        manager.SetList(
            {"tls_config", "mindie_mangment_tls_items", "tls_cert"},
            GetAbsolutePath(certDir,
                            "coordinator_to_mindie_server_manage/cert.pem"));
        manager.SetList(
            {"tls_config", "mindie_mangment_tls_items", "tls_key"},
            GetAbsolutePath(
                certDir, "coordinator_to_mindie_server_manage/cert.key.pem"));
        manager.SetList(
            {"tls_config", "mindie_mangment_tls_items", "tls_passwd"},
            GetAbsolutePath(
                certDir,
                "coordinator_to_mindie_server_manage/cert_passwd.txt"));
        manager.Save();
    }

    {
        ...
        manager.SetList(
            {"tls_config", "request_coordinator_tls_items", "ca_cert"},
            GetAbsolutePath(certDir, "controller_to_coordinator/ca.pem"));
        manager.SetList(
            {"tls_config", "request_coordinator_tls_items", "tls_cert"},
            GetAbsolutePath(certDir, "controller_to_coordinator/cert.pem"));
        manager.SetList(
            {"tls_config", "request_coordinator_tls_items", "tls_key"},
            GetAbsolutePath(certDir, "controller_to_coordinator/cert.key.pem"));
        manager.SetList(
            {"tls_config", "request_coordinator_tls_items", "tls_passwd"},
            GetAbsolutePath(certDir,
                            "controller_to_coordinator/cert_passwd.txt"));
        manager.SetList(
            {"tls_config", "request_coordinator_tls_items", "tls_crl"}, "");
        manager.Save();
    }
    ...
    for (auto index = 0; index < numMindIEServer; index++) {
        ...
        threads.emplace_back(std::thread([&] {
            CreateSSLServer(predictIPList[index], std::to_string(predictPort),
                            predictIPList[index], std::to_string(managePort), 4,
                            std::to_string(metricsPort));  // 4 线程
        }));
        sleep(5);
    }
    ...
    threadCoordinator.detach();
    threadController.detach();
    std::cout << "---------------------StressTetsPDSSLHelper Test end "
                 "----------------------"
              << std::endl;
...
// 单机场景压测
void StressTetsSingleNodeHelper(uint32_t numMindIEServer, ReqTestFun reqTestFun,
                                MetricsReqTestFun metricsFun,
                                uint32_t numThreadInfer = 10) {
    std::cout << "---------------------StressTetsSingleNodeHelper Test start "
                 "----------------------"
              << std::endl;
    ...
    // 修改启动配置文件
    {
        ...
        manager.SetList({"http_config", "manage_port"},
                        std::to_string(managePort));
        manager.SetList({"digs_scheduler_config", "deploy_mode"},
                        "single_node");
    ...
    std::string stressSingleNodeData = GetAbsolutePath(
        parentPath,
        "tests/ms/common/.mindie_ms/ms_metricsStressSingleNodeData.bin");
    ...
    threadCoordinator.detach();
    threadController.detach();
    std::cout << "---------------------StressTetsSingleNodeHelper Test end "
                 "----------------------"
              << std::endl;
    sleep(5);  // 停5s，等待后台的所有操作结束
}
