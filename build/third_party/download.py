...
# Save third-party zipped packages.
THIRD_PARTY_ZIP_DIR = os.environ.get("THIRD_PARTY_ZIP_DIR", "zipped")
# Check if add `--no-check-certificate` when use `wget`
NO_CHECK_CERTIFICATE = os.environ.get("NO_CHECK_CERTIFICATE", "False")
NO_CHECK_CERTIFICATE = NO_CHECK_CERTIFICATE.strip().lower() in ("1", "true")

logging.basicConfig(level=logging.INFO)


def parse_dependencies(json_file_path: str) -> List[str]:
...
def check_integrity(file_path: str, sha256="") -> bool:
    ...
    """
    if not sha256:
        logging.warning(f"Sha256sum for {file_path} is empty, skip to compare the integrity.")
        return True
